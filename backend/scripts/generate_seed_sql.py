...
sql_statements = []
sql_statements.append('INSERT INTO levels (level_number, title, cefr_band, description, required_vocab_count, unlock_score_threshold) VALUES')

values = []
for i in range(100):
    level_number = i + 1
    cefr = 'A1'
    if level_number > 80: cefr = 'C2'
    elif level_number > 60: cefr = 'C1'
    elif level_number > 45: cefr = 'B2'
    elif level_number > 30: cefr = 'B1'
    elif level_number > 15: cefr = 'A2'
    
    title = LEVEL_TITLES[i].replace("'", "''") if i < len(LEVEL_TITLES) else f'Advanced English Level {level_number}'
    description = f'Master key sentence structures, idioms, and fluent vocabulary tailored for {cefr} level proficiency.'
    req_vocab = 15 + (level_number % 10) * 2
    
    values.append(f"({level_number}, '{title}', '{cefr}', '{description}', {req_vocab}, 80.0)")

sql_statements.append(',\n'.join(values) + '\nON CONFLICT (level_number) DO NOTHING;')

with open('../supabase/seed.sql', 'w') as f:
    f.write('\n'.join(sql_statements) + '\n')
print('Generated ../supabase/seed.sql')
