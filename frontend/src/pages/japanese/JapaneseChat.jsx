...
const SCENARIOS = [
  { 
    ...
    icon: MessageSquare, 
    promptPrefix: "[Japanese Language Tutor Mode] You are teaching Japanese to an English speaker. For every reply, provide the Japanese (Kana/Kanji) first, followed by the English transliteration (Romaji) in parentheses, and then the English translation. Example format: 'こんにちは！ (Konnichiwa!) - Hello!'. Gently correct any mistakes. User says: ",
    welcome: () => `こんにちは！ (Konnichiwa!) - Hello! I am Gemma, your Japanese conversation coach. You can speak in English, and I will help you translate it to Japanese, or you can try speaking in Japanese!`
  },
  { 
    id: "interview", 
    label: "Job Interview Roleplay", 
    icon: Briefcase, 
    promptPrefix: "[Japanese Job Interview Tutor Mode] You are teaching formal Japanese to an English speaker. Provide the Japanese, then (transliteration), then - English translation. User says: ",
    welcome: () => `面接の練習へようこそ！ (Mensetsu no renshū e yōkoso!) - Welcome to the interview practice! I will act as your hiring manager. Tell me about yourself.`
  },
  { 
    id: "business", 
    label: "Business Presentation", 
    icon: Award, 
    promptPrefix: "[Japanese Business Tutor Mode] You are teaching professional Japanese to an English speaker. Provide the Japanese, then (transliteration), then - English translation. User says: ",
    welcome: () => `ビジネスプレゼンテーションへようこそ！ (Bijinesu purezentēshon e yōkoso!) - Welcome to the Business Presentation practice! What would you like to present today?`
  },
  { 
    id: "travel", 
    label: "Travel & Dining Out", 
    icon: Compass, 
    promptPrefix: "[Japanese Travel Tutor Mode] You are teaching conversational travel Japanese to an English speaker. Provide the Japanese, then (transliteration), then - English translation. User says: ",
    welcome: () => `旅行と外食の練習へようこそ！ (Ryokō to gaishoku no renshū e yōkoso!) - Welcome to Travel & Dining Out practice! Imagine you're at a hotel in Tokyo. What do you say?`
  },
];

export default function JapaneseChat() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("lingolive_japanese_chat_history");
      if (saved) {
  ...
  const saveSessionToHistory = (currentMsgs, scenarioId, currentId) => {
    ...
    setHistoryList((prev) => {
      ...
      try {
        localStorage.setItem("lingolive_japanese_chat_history", JSON.stringify(updated));
      } catch (e) {
  ...
  const handleClearAllHistory = () => {
    if (window.confirm("Clear all chat history?")) {
      setHistoryList([]);
      localStorage.removeItem("lingolive_japanese_chat_history");
    }
  };
  ...
  const handleSendMessage = async (e, customText = null) => {
    ...
    try {
      ...
      // Attaching the provided API Key per user request for Japanese realtime
      const API_KEY = import.meta.env.VITE_JAPANESE_REALTIME_API_KEY || "";
  ...
  return (
    <div className="relative mx-auto max-w-5xl h-[calc(100vh-7.5rem)] flex flex-col rounded-3xl border border-white/10 bg-[#050816] text-white shadow-2xl overflow-hidden font-sans">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 bg-gradient-to-r from-red-950/80 via-[#0f172a] to-[#050816] px-6 py-4 backdrop-blur-xl z-10">
        <div className="flex items-center gap-3.5">
          <div className="relative">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-tr from-red-600 to-rose-400 text-white shadow-lg shadow-red-500/30">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-rose-400/10 border border-rose-400/20 px-2.5 py-0.5 font-mono text-[10px] font-semibold text-rose-300">
                <Sparkles className="h-3 w-3 text-rose-400" /> Real-Time Gemini AI
              </span>
            </div>
            <p className="font-mono text-xs text-slate-400">
              Real-Time Japanese Conversation Coach • Live Voice &amp; Text
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-rose-400/30 bg-rose-500/10 px-3.5 py-1.5 text-xs font-bold text-rose-300 hover:bg-rose-500/20 transition-all shadow-sm"
            title="Open Chat History"
          >
            <History className="h-4 w-4 text-rose-400" />
            <span>History</span>
            {historyList.length > 0 && (
              <span className="ml-1 rounded-full bg-rose-400 text-[#050816] px-1.5 py-0.2 font-mono text-[10px] font-extrabold">
                {historyList.length}
              </span>
            )}
          </button>
          ...
          >
            {autoSpeak ? <Volume2 className="h-3.5 w-3.5 text-rose-400" /> : <VolumeX className="h-3.5 w-3.5" />}
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 font-medium transition-all shrink-0 ${
                  isSelected
                    ? "bg-red-600/30 border border-red-500/40 text-rose-300 shadow-sm"
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 custom-scrollbar bg-radial-gradient">
              ...
              <div
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl font-bold shadow-md ${
                  msg.sender === "user"
                    ...
                    : "bg-gradient-to-tr from-red-900 to-slate-800 border border-white/10 text-rose-400"
                }`}
              >
        ...
        {loading && (
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-red-900/50 text-rose-400 border border-white/10">
      ...
      <div className="border-t border-white/5 bg-[#050816]/90 px-6 py-2 flex items-center gap-2 overflow-x-auto custom-scrollbar">
        ...
        {suggestionChips.map((chip, idx) => (
          <button
            ...
            disabled={loading}
            className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] text-slate-300 hover:border-rose-400/40 hover:bg-white/10 hover:text-white transition-all shrink-0"
          >
            {chip}
          </button>
        ))}
      ...
      <form onSubmit={(e) => handleSendMessage(e)} className="border-t border-white/10 bg-[#0f172a]/95 p-4 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          >
            {isListening ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5 text-rose-400" />}
          </button>
          ...
          <input
            ...
            placeholder={isListening ? "Listening... speak now..." : "Type your sentence in English or Japanese…"}
            className="flex-1 rounded-2xl border border-white/10 bg-white/5 px-4 py-3.5 font-sans text-sm text-white placeholder-slate-500 outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-400/20 transition-all"
          />
          ...
          <button
            ...
            className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-r from-red-600 to-rose-500 text-white shadow-lg shadow-red-600/30 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 shrink-0"
          >
          ...
          >
            ...
            >
              <div>
                ...
                <button
                  onClick={handleStartNewChat}
                  className="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-red-600 to-rose-500 py-3 font-sans text-sm font-bold text-white shadow-lg shadow-red-600/30 hover:scale-[1.02] transition-all mb-6"
                >
                    ...
                    historyList.map((item) => (
                      <div
                        ...
                        className={`group relative flex items-center justify-between p-3.5 rounded-2xl border transition-all cursor-pointer ${
                          item.id === sessionId
                            ? "bg-rose-500/15 border-rose-400/40 text-white"
                            : "bg-white/5 border-white/5 text-slate-300 hover:bg-white/10"
                        }`}
                      >
                        <div className="flex items-center gap-3 overflow-hidden">
                          <div className="h-9 w-9 rounded-xl bg-red-600/20 text-rose-400 flex items-center justify-center shrink-0">
                            <MessageCircle className="h-4 w-4" />
                          </div>
                          <div className="overflow-hidden text-left">
                            <span className="block font-mono text-[10px] font-bold text-rose-400 uppercase tracking-wider">
                              {item.scenarioLabel}
                            </span>
