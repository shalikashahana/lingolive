              ...
              <div className="space-y-6">
                {activeSentenceModuleView === null ? (
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="flex items-center justify-between">
                      <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                        <BookOpen className="w-6 h-6 text-[#f59e0b]" /> Module {hindiSentencesData.modules[activeSentenceModuleView].module} ({hindiSentencesData.modules[activeSentenceModuleView].total_sentences} Sentences)
                      </h3>
                    </div>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(hindiSentencesData.modules[activeSentenceModuleView].sentences.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = Math.min((i + 1) * 10, hindiSentencesData.modules[activeSentenceModuleView].sentences.length);
                        const partName = `Part ${i + 1} (${startIdx + 1}-${endIdx})`;
                        
                        let moduleGlobalStartIdx = 0;
                        for (let m = 0; m < activeSentenceModuleView; m++) {
                           moduleGlobalStartIdx += hindiSentencesData.modules[m].total_sentences;
                        }
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="flex items-center justify-between">
                      <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                        <BookOpen className="w-6 h-6 text-[#f59e0b]" /> Module {hindiSentencesData.modules[activeSentenceModuleView].module} - Part {activeSentencePartView + 1}
                      </h3>
                    </div>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {hindiSentencesData.modules[activeSentenceModuleView].sentences.slice(activeSentencePartView * 10, (activeSentencePartView + 1) * 10).map((sentence, relIdx) => {
                         let globalIdx = 0;
                         for (let m = 0; m < activeSentenceModuleView; m++) {
                            globalIdx += hindiSentencesData.modules[m].total_sentences;
                         }
                         ...
                         return (
                           <WordCard 
                             ...
                             isInProgress={isInProgress}
                             isLocked={isLocked}
                             onInteract={() => handleInteraction('sentences', globalIdx, sentence.hindi)} 
                           />
                         );
                      })}
                    </div>
