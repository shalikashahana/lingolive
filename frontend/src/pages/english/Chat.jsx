...
const SCENARIOS = [
  { 
    ...
    icon: MessageSquare, 
    promptPrefix: "",
    welcome: (lvl) => `Hello! I am Gemma, your real-time AI Conversational English Coach. We are practicing for Level ${lvl}. Share an idea or start speaking, and I'll give you natural feedback and grammar tips!`
  },
  { 
    id: "interview", 
    label: "Job Interview Roleplay", 
    icon: Briefcase, 
    promptPrefix: "[Job Interview Scenario] ",
    welcome: (lvl) => `Welcome to Job Interview Roleplay (Level ${lvl})! I will act as your hiring manager. Tell me about yourself and your professional strengths!`
  },
  { 
    id: "business", 
    label: "Business Presentation", 
    icon: Award, 
    promptPrefix: "[Business Presentation Scenario] ",
    welcome: (lvl) => `Welcome to Business Presentation Practice! Present your project proposal or corporate ideas, and I'll guide your formal vocabulary and executive delivery.`
  },
  { 
    id: "travel", 
    label: "Travel & Dining Out", 
    icon: Compass, 
    promptPrefix: "[Travel Scenario] ",
    welcome: (lvl) => `Welcome to Travel & Dining Out Practice! Imagine you've arrived at an international airport or hotel. Ask for directions or order food to begin!`
  },
];
...
export default function Chat() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("lingolive_chat_history");
      if (saved) {
  ...
  const saveSessionToHistory = (currentMsgs, scenarioId, currentId) => {
    ...
    setHistoryList((prev) => {
      ...
      try {
        localStorage.setItem("lingolive_chat_history", JSON.stringify(updated));
      } catch (e) {
  ...
  const handleClearAllHistory = () => {
    if (window.confirm("Clear all chat history?")) {
      setHistoryList([]);
      localStorage.removeItem("lingolive_chat_history");
    }
  };
      ...
      <form onSubmit={(e) => handleSendMessage(e)} className="border-t border-white/10 bg-[#0f172a]/95 p-4 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            placeholder={isListening ? "Listening... speak now..." : "Type your sentence or question in English…"}
