    ...
    <div className={`group relative flex flex-col p-5 bg-white/80 backdrop-blur-xl rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-md overflow-hidden h-full ${
      ...
      <div className="flex-1">
        <span className="text-[22px] font-bold font-sans leading-[1.7] tracking-wide text-[#14213D] mb-3 pr-2 flex items-start gap-2 break-words">
          ...
          <span className={isLocked ? "blur-[2px] opacity-70" : ""}>{word.korean}</span>
        </span>
  ...
  return (
    <div className={`group relative flex flex-col p-5 bg-white/80 backdrop-blur-xl rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-md overflow-hidden h-full ${
      ...
      <div className="flex justify-between items-start mb-2">
        ...
        <button 
          onClick={(e) => { e.stopPropagation(); playAudio(question.korean); }}
          disabled={isLocked}
          className={`p-1.5 rounded-xl shadow-sm border transition-all z-10 hover:scale-110 active:scale-95 ${
      ...
      <div className="flex-1 mb-4">
        <span className="text-[22px] font-bold font-sans leading-[1.7] tracking-wide text-[#14213D] mb-3 pr-2 flex items-start gap-2 break-words">
          ...
          <span className={isLocked ? "blur-[2px] opacity-70" : ""}>{question.korean}</span>
        </span>
              ...
              <div className="space-y-8 animate-fade-in">
                <div className="space-y-2">
                  ...
                  <p className="font-sans text-sm text-[#14213D]/60 max-w-xl">
                    Pick up where you left off or start a new lesson. Your Korean journey is waiting for you!
                  </p>
                </div>
                ...
                {activeWordPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="font-display text-xl font-bold text-[#14213D] flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-[#0ea5e9]" /> {koreanWordsData.words.length} Essential Words
                    </h3>
            ...
            {activeTab === "Numbers" && (
              <div className="space-y-6">
                {activeNumberPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="font-display text-xl font-bold text-[#14213D] flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-[#ec4899]" /> {koreanNumbersData.numbers.length} Numbers
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            ...
            {activeTab === "Sentences" && (
              <div className="space-y-6">
                {activeSentenceModuleView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="font-display text-xl font-bold text-[#14213D] flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-[#f59e0b]" /> {koreanSentencesData.total_sentences} Sentences
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {koreanSentencesData.modules.map((moduleData, i) => (
                        <button
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="flex items-center justify-between">
                      <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                        <BookOpen className="w-6 h-6 text-[#f59e0b]" /> Module {koreanSentencesData.modules[activeSentenceModuleView].module} ({koreanSentencesData.modules[activeSentenceModuleView].total_sentences} Sentences)
                      </h3>
                    </div>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(koreanSentencesData.modules[activeSentenceModuleView].sentences.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = Math.min((i + 1) * 10, koreanSentencesData.modules[activeSentenceModuleView].sentences.length);
                        const partName = `Part ${i + 1} (${startIdx + 1}-${endIdx})`;
                        
                        let moduleGlobalStartIdx = 0;
                        for (let m = 0; m < activeSentenceModuleView; m++) {
                           moduleGlobalStartIdx += koreanSentencesData.modules[m].total_sentences;
                        }
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="flex items-center justify-between">
                      <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                        <BookOpen className="w-6 h-6 text-[#f59e0b]" /> Module {koreanSentencesData.modules[activeSentenceModuleView].module} - Part {activeSentencePartView + 1}
                      </h3>
                    </div>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {koreanSentencesData.modules[activeSentenceModuleView].sentences.slice(activeSentencePartView * 10, (activeSentencePartView + 1) * 10).map((sentence, relIdx) => {
                         let globalIdx = 0;
                         for (let m = 0; m < activeSentenceModuleView; m++) {
                            globalIdx += koreanSentencesData.modules[m].total_sentences;
                         }
                         ...
                         return (
                           <WordCard 
                             ...
                             isInProgress={isInProgress}
                             isLocked={isLocked}
                             onInteract={() => handleInteraction('sentences', globalIdx, sentence.korean)} 
                           />
                         );
                      })}
                    </div>
