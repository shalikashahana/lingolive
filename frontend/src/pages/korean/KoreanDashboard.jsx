...
import { 
  ...
  Play, Volume2, Eye, EyeOff, User, LogOut, Lock, Star, Flame, Zap, BarChart3, Globe, LayoutDashboard, Search, MessageCircle, ChevronDown, ChevronUp, Clock, Target, Award
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import CatVoiceCheckpoint from "../../components/catTeacher/CatVoiceCheckpoint";
import KoreanChat from "./KoreanChat";

function ProgressRing({ progress, size = 44, strokeWidth = 4, color = "#FF6B6B" }) {
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <span className="absolute font-mono text-[10px] font-bold text-rose-300">
        {Math.round(progress)}%
      </span>
    </div>
  );
  ...
  return (
    <motion.div 
      ...
      className={`group relative flex flex-col p-5 bg-[#0f172a]/90 backdrop-blur-xl rounded-3xl border transition-all duration-300 overflow-hidden h-full shadow-lg ${
        isInProgress ? "border-rose-500 ring-2 ring-rose-500/30 bg-rose-950/20" : 
    ...
    >
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center gap-1.5">
          ...
          {isInProgress && <span className="flex items-center gap-1 text-[11px] font-mono font-bold text-rose-400 bg-rose-500/10 px-2.5 py-0.5 rounded-full border border-rose-500/20"><Play className="w-3 h-3 animate-pulse" /> Active</span>}
        ...
        <button 
          ...
          className={`p-2 rounded-2xl border transition-all ${
            isLocked ? "bg-white/5 border-white/5 opacity-40 cursor-not-allowed" : "bg-blue-500/10 border-blue-500/30 text-blue-400 hover:bg-blue-500 hover:text-white hover:scale-110 active:scale-95"
          }`}
        >
      ...
      <div className="flex-1">
        <div className="text-2xl font-bold font-sans text-white mb-2 flex items-start gap-2 break-words">
          {word.digit && (
            <span className="mt-1 flex-shrink-0 bg-blue-500/20 border border-blue-500/30 text-blue-300 px-2 py-0.5 rounded-lg text-xs font-mono font-bold">
              {word.digit}.
            </span>
          )}
          <span className={isLocked ? "blur-[3px] opacity-50" : "text-blue-100"}>{word.korean}</span>
        </div>
        
        <div className={`flex flex-wrap gap-2 mb-4 ${isLocked ? "opacity-40" : ""}`}>
          <span className="font-mono text-[11px] font-semibold bg-white/5 border border-white/10 text-rose-300 px-2.5 py-1 rounded-xl">
...
export default function KoreanDashboard() {
  ...
  const savedTab = localStorage.getItem("korean_active_tab");
  const [activeTab, setActiveTabState] = useState(savedTab || "Home");

  const setActiveTab = (tab) => {
    setActiveTabState(tab);
    localStorage.setItem("korean_active_tab", tab);
  };
  ...
  useEffect(() => {
    const savedProgressStr = localStorage.getItem("korean_progress");
    const defaultProgress = { swarangal: 0, vyanjanangal: 0, words: 0, numbers: 0, sentences: 0, quiz: 0 };
  ...
  const handleInteraction = (type, index, text) => {
    ...
    if (index === progress[type]) {
      ...
      localStorage.setItem("korean_progress", JSON.stringify(newProgress));
  ...
  const formattedKoreanSentences = useMemo(() => {
    const parts = [];
    koreanSentencesData.modules.forEach((mod, modIndex) => {
  ...
  const [completedPhases, setCompletedPhases] = useState(() => {
    const saved = localStorage.getItem("korean_completed_phases");
  ...
  const handleCheckpointComplete = (score, total) => {
    if (checkpointPhase) {
      ...
      localStorage.setItem("korean_completed_phases", JSON.stringify(updated));
    }
  };

  const filteredData = useMemo(() => {
    let dataToFilter = formattedKoreanSentences;
    if (!searchQuery.trim()) {
    ...
    return dataToFilter.map(phaseObj => {
      const matchingSentences = phaseObj.sentences.filter(s => {
        ...
        const koMatch = s.korean.includes(searchQuery);
        return enMatch || taMatch || koMatch;
      });
      return { ...phaseObj, sentences: matchingSentences };
    }).filter(phaseObj => phaseObj.sentences.length > 0);
  }, [searchQuery, formattedKoreanSentences, activeSentenceModuleView, activeSentenceLevelView]);
  ...
  // Korean Seoul Blue + Coral Visual Theme Cards
  const dashboardCards = [
    { key: "swarangal", label: "Vowels (모음)", native: "모음", tab: TABS[1], icon: "ㅏ", total: alphabetData.alphabet.swarangal.length, color: "#2563EB", estimatedTime: "15 mins", difficulty: "Beginner" },
    { key: "vyanjanangal", label: "Consonants (자음)", native: "자음", tab: TABS[1], icon: "ㄱ", total: alphabetData.alphabet.vyanjanangal.length, color: "#FF6B6B", estimatedTime: "30 mins", difficulty: "Beginner" },
    { key: "words", label: "Essential Words", native: "단어", tab: TABS[2], icon: "📚", total: koreanWordsData.words.length, color: "#0EA5E9", estimatedTime: "45 mins", difficulty: "Beginner" },
    { key: "numbers", label: "Numbers", native: "숫자", tab: TABS[3], icon: "🔢", total: koreanNumbersData.numbers.length, color: "#EC4899", estimatedTime: "25 mins", difficulty: "Beginner" },
    { key: "sentences", label: "Sentences", native: "문장", tab: TABS[4], icon: "💬", total: koreanSentencesData.total_sentences, color: "#10B981", estimatedTime: "60 mins", difficulty: "Intermediate" },
    { key: "quiz", label: "Quiz Dashboard", native: "퀴즈", tab: TABS[5], icon: "🧠", total: koreanQuizData.total_questions, color: "#8B5CF6", estimatedTime: "20 mins", difficulty: "All Levels" }
  ];
        ...
        return (
          <motion.div 
            ...
            className={`group relative flex flex-col items-center justify-between aspect-square p-3 rounded-2xl border transition-all duration-300 overflow-hidden ${
              isInProgress
                ? "border-rose-500 bg-rose-500/10 ring-2 ring-rose-500/40 shadow-lg cursor-pointer"
                : isCompleted
          ...
          >
            <div className="flex w-full items-center justify-between z-10">
               ...
               {isInProgress && <Play className="w-3.5 h-3.5 text-rose-400 fill-rose-400 animate-pulse" />}
               {isLocked && <Lock className="w-3.5 h-3.5 text-slate-500" />}
            </div>

            <div className="my-1 flex h-12 w-12 items-center justify-center rounded-2xl font-mono text-base font-bold bg-white/5 text-blue-300 border border-white/10 shadow-sm group-hover:border-blue-500/40 transition-colors">
              <span className={`text-3xl font-bold font-sans ${isLocked ? 'text-slate-600' : 'text-blue-200 group-hover:text-blue-400'} transition-colors`}>
                {letter.letter}
              </span>
            </div>
            
            <div className="flex flex-col items-center gap-1 z-10 w-full px-1">
              <span className="font-mono text-[10px] font-semibold bg-white/5 text-rose-300 px-1.5 py-0.5 rounded-lg w-full text-center truncate border border-white/5">
  ...
  return (
    <div className="flex h-screen bg-[#050816] overflow-hidden font-sans text-white">
      {/* Sidebar */}
      <aside className="w-64 h-screen bg-[#090d1f]/95 border-r border-blue-500/10 flex flex-col backdrop-blur-2xl shrink-0">
        <div className="p-5 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-tr from-blue-600 to-rose-400 text-white shadow-lg shadow-blue-500/20 font-bold">
                ...
                {availableLanguages.map((lang) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 px-4 py-2.5 font-sans text-sm font-semibold transition-colors ${
                      currentLanguageCode === lang.code ? "bg-blue-500/10 text-blue-400" : "text-slate-300 hover:bg-white/5"
                    }`}
                  >
                    <span>{lang.flag}</span>
                    <span>{lang.name}</span>
        ...
        <div className="px-6 pt-3 pb-1">
          <span className="font-mono text-[10px] font-bold text-blue-400/60 uppercase tracking-widest">Core Modules</span>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-1 custom-scrollbar">
          {TABS.map((tabName) => {
            const getIcon = (name) => {
              ...
              if (name.includes("Alphabets")) return <span className="text-sm font-sans font-bold">한</span>;
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-2xl font-semibold text-sm transition-all duration-200 ${
                  isActive
                    ? "bg-gradient-to-r from-blue-600/30 to-rose-500/10 border border-blue-500/40 text-blue-300 shadow-sm"
                    : "text-slate-400 hover:bg-white/5 hover:text-white border border-transparent"
                }`}
              >
                <div className="flex items-center gap-3">
                  <span className={isActive ? "text-blue-400" : "text-slate-500"}>{getIcon(tabName)}</span>
                  <span className="truncate text-left">{tabName}</span>
                </div>
                {isActive && <div className="h-2 w-2 rounded-full bg-rose-400 shadow-[0_0_8px_#ff6b6b] shrink-0" />}
              </button>
            );
          })}
        </nav>
        ...
        <div className="p-4 border-t border-white/10 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div className="flex items-center gap-2 rounded-xl bg-rose-500/10 border border-rose-500/20 px-3 py-2">
              <Flame className={`w-4 h-4 ${stats.streak > 0 ? "text-rose-400 fill-rose-400" : "text-slate-500"}`} />
              <div>
                <div className="font-bold text-sm text-rose-300 leading-none">{stats.streak}d</div>
                <div className="text-[9px] font-mono text-rose-600 uppercase tracking-wider mt-0.5">Streak</div>
              </div>
            </div>
            <div className="flex items-center gap-2 rounded-xl bg-blue-500/10 border border-blue-500/20 px-3 py-2">
              <Zap className={`w-4 h-4 ${stats.xp > 0 ? "text-blue-400 fill-blue-400" : "text-slate-500"}`} />
              <div>
                <div className="font-bold text-sm text-blue-300 leading-none">{stats.xp}</div>
                <div className="text-[9px] font-mono text-blue-600 uppercase tracking-wider mt-0.5">XP</div>
              </div>
            </div>
          </div>
          ...
          {user && (
            <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-2.5">
              <div className="flex items-center gap-2.5 overflow-hidden">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-tr from-blue-600 to-rose-400 text-xs font-bold text-white">
                  {user.displayName?.[0] || user.email?.[0]?.toUpperCase() || "K"}
                </div>
                <div className="overflow-hidden">
                  <p className="text-xs font-bold text-white truncate">{user.displayName || "Korean Learner"}</p>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto bg-[#050816] custom-scrollbar">
        <div className="p-6 sm:p-8 max-w-6xl mx-auto w-full space-y-8 pb-16">
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 p-6 rounded-3xl border border-blue-500/20 bg-gradient-to-br from-[#0f172a] to-[#090d1f] flex flex-col justify-between space-y-4 shadow-xl">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-3 py-1 rounded-full bg-rose-500/10 text-rose-300 font-mono text-xs font-bold border border-rose-500/20">+50 XP Reward</span>
                    </div>

                    <div className="space-y-3">
                      {[
                        { text: "Learn 5 Hangul Vowels (모음)", tab: "Alphabets (한글)", done: progress.swarangal >= 5 },
                        { text: "Study 10 Essential Korean Words", tab: "Essential Words", done: progress.words >= 10 },
                        { text: "Take Korean Practice Quiz", tab: "Quiz Dashboard", done: false },
                      ].map((item, i) => (
                        <div
                          key={i}
                  ...
                  <div className="p-6 rounded-3xl border border-rose-500/20 bg-gradient-to-b from-rose-950/30 via-[#0f172a] to-[#050816] flex flex-col justify-between space-y-4 shadow-xl">
                    <div>
                      <div className="h-10 w-10 rounded-2xl bg-gradient-to-tr from-blue-600 to-rose-400 flex items-center justify-center text-white shadow-md mb-3 font-bold">
                        <Sparkles className="w-5 h-5" />
                      </div>
                      <h3 className="text-lg font-bold text-white mb-1">Continue Learning</h3>
                      <p className="text-xs text-slate-400 leading-relaxed">
                        Keep building Korean vocabulary and reading fluency.
                      </p>
                    </div>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-5">
                    <h2 className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">Korean Modules</h2>
                    ...
                    {dashboardCards.map((card) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className="group relative text-left p-6 rounded-3xl border border-white/10 bg-[#0f172a]/90 flex flex-col justify-between overflow-hidden transition-all duration-300 hover:border-blue-500/40 hover:shadow-2xl shadow-lg"
                        >
                          <div className="flex items-center justify-between mb-6 w-full">
                            <div className="h-12 w-12 rounded-2xl flex items-center justify-center text-xl font-bold text-white shadow-md bg-blue-500/20 border border-blue-500/30">
                              {card.icon}
                            </div>
                            <span className="font-mono text-[10px] font-bold uppercase tracking-wider text-rose-400 bg-rose-500/10 px-2.5 py-1 rounded-full border border-rose-500/20">
                              {card.difficulty}
                            </span>
                          </div>
                          ...
                          <div className="w-full space-y-2">
                            <div className="flex items-baseline justify-between">
                              ...
                              <span className="text-xs font-sans text-rose-400 font-bold">{card.native}</span>
                            </div>
                            ...
                            <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
                              <div
                                className="h-full rounded-full bg-gradient-to-r from-blue-600 to-rose-400 transition-all duration-700"
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          </div>
            ...
            {activeTab === "Alphabets (한글)" && (
              <div className="space-y-10">
                <div className="space-y-4">
                  <h3 className="text-2xl font-bold text-white flex items-center gap-2">
                    <Languages className="w-6 h-6 text-blue-400" /> Vowels (모음)
                  </h3>
                ...
                <div className="space-y-4">
                  <h3 className="text-2xl font-bold text-white flex items-center gap-2">
                    <Languages className="w-6 h-6 text-rose-400" /> Consonants (자음)
                  </h3>
                ...
                {activeWordPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-sky-400" /> {koreanWordsData.words.length} Essential Korean Words
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(koreanWordsData.words.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = (i + 1) * 10;
                        ...
                        return (
                          ...
                          >
                            <BookOpen className={`w-8 h-8 mb-3 ${isLocked ? "text-slate-600" : isInProgress ? "text-rose-400" : "text-emerald-400"}`} />
                            ...
                            <div className="mt-3">
                              {isLocked ? <Lock className="w-5 h-5 text-slate-600" /> : 
                               ...
                               <span className="text-xs font-mono font-bold text-rose-400 bg-rose-500/20 px-3 py-1 rounded-full border border-rose-500/30">In Progress</span>}
                            </div>
                          </button>
                        );
                      })}
                  ...
                  <div className="space-y-6 pt-4">
                    <button 
                      ...
                      className="flex items-center gap-2 text-xs font-bold text-blue-300 bg-white/5 hover:bg-white/10 px-4 py-2 rounded-2xl border border-white/10 transition-colors"
                    >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {koreanWordsData.words.slice(activeWordPartView * 10, (activeWordPartView + 1) * 10).map((word, relIdx) => {
                         ...
                         return (
                           <WordCard 
                             ...
                             isLocked={isLocked}
                             onInteract={() => handleInteraction('words', globalIdx, word.korean)} 
                           />
                         );
                      })}
                    </div>
            ...
            {activeTab === "Numbers" && (
              <div className="space-y-6">
                {activeNumberPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-pink-400" /> {koreanNumbersData.numbers.length} Korean Numbers
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(koreanNumbersData.numbers.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = (i + 1) * 10;
                        const partName = `Part ${i + 1} (${startIdx + 1}-${endIdx})`;
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {koreanNumbersData.numbers.slice(activeNumberPartView * 10, (activeNumberPartView + 1) * 10).map((number, relIdx) => {
                         ...
                         return (
                           <WordCard 
                             ...
                             isInProgress={isInProgress}
                             isLocked={isLocked}
                             onInteract={() => handleInteraction('numbers', globalIdx, number.korean)} 
                           />
                         );
                      })}
                    </div>
            ...
            {activeTab === "Sentences" && (
              <div className="space-y-8 pb-16 w-full max-w-4xl mx-auto">
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-950/40 via-[#0f172a] to-[#050816] p-8 border border-blue-500/30 shadow-2xl">
                  <h1 className="text-3xl font-extrabold text-white mb-2">Daily Conversations (문장)</h1>
                  <p className="text-sm text-slate-400">Master everyday Korean sentences grouped by practical scenarios.</p>
                </div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Search sentences in Korean, English or Tamil..."
                    className="w-full rounded-2xl border border-white/10 bg-slate-900/80 py-3.5 pl-12 pr-4 font-sans text-sm text-white placeholder-slate-500 outline-none focus:border-blue-400"
                  />
                </div>
                ...
                <div className="space-y-4">
                  {activeSentenceModuleView === null && !searchQuery.trim() ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from(new Set(formattedKoreanSentences.map(p => p.moduleIndex))).map((moduleIdx) => {
                        const moduleData = formattedKoreanSentences.filter(p => p.moduleIndex === moduleIdx);
                        ...
                        return (
                          <button
                            ...
                            className="flex flex-col items-center justify-center p-8 rounded-3xl border border-blue-500/30 bg-blue-500/10 hover:bg-blue-500/20 transition-all duration-300 shadow-lg"
                          >
                            <MessageCircle className="w-8 h-8 mb-3 text-blue-400" />
                            <span className="text-lg font-bold text-white">Module {moduleName}</span>
                            <div className="mt-3">
                              <span className="text-xs font-mono font-bold text-blue-400 bg-blue-500/20 px-3 py-1 rounded-full border border-blue-500/30">{totalSentences} Sentences</span>
                            </div>
                          </button>
                        );
                      })}
                    ...
                    <div className="space-y-6 pt-4">
                      <button 
                        ...
                        className="mb-4 flex items-center gap-2 text-xs font-bold text-blue-300 bg-white/5 hover:bg-white/10 px-4 py-2 rounded-2xl border border-white/10 transition-colors"
                      >
                        ...
                        {Array.from(new Set(filteredData.map(p => p.levelIndex))).map((levelIdx) => {
                          ...
                          return (
                            <button
                              ...
                              className="flex flex-col items-center justify-center p-6 rounded-3xl border border-blue-500/20 bg-blue-500/5 hover:bg-blue-500/10 transition-all duration-300 shadow-sm"
                            >
                          ...
                          <div key={data.phase} className="space-y-4">
                            ...
                            <div className="mb-6">
                              <button
                                ...
                                className="w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-500/20 to-blue-900/40 border border-blue-500/30 p-4 font-bold text-blue-300 shadow-md hover:shadow-lg transition-all hover:bg-blue-500/30"
                              >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {data.sentences.map((sentence, sIndex) => {
                                ...
                                return (
                                  <div key={sIndex} className="p-5 rounded-2xl bg-[#0f172a]/80 backdrop-blur-sm border border-white/10 hover:border-blue-500/30 transition-all shadow-lg flex flex-col justify-between min-h-[140px]">
                                    <div className="flex items-start justify-between mb-4">
                                      <p className="text-xl font-bold text-blue-200 leading-relaxed pr-4">{sentence.korean}</p>
                                      <button onClick={() => playAudio(sentence.korean)} className="p-2.5 rounded-xl bg-blue-500/10 text-blue-400 hover:bg-blue-500 hover:text-white transition-all shrink-0">
                                        <Volume2 className="h-4 w-4" />
                                      </button>
                                    </div>
                                    <div>
                                      <button onClick={() => toggleTranslation(sIndex, data.phase)} className="text-xs font-bold text-rose-400 hover:text-rose-300 transition-colors">
                                      ...
                                      {isTranslated && (
                                        <div className="pt-3 mt-2 border-t border-white/10 text-xs space-y-1.5 text-slate-300 animate-fade-in">
                                          ...
                                          <p className="italic font-mono text-blue-300/80">{sentence.transliteration}</p>
                                        </div>
                                      )}
                                    </div>
                                  </div>
            ...
            {activeTab === "Quiz" && (
              <div className="-mx-4 sm:-mx-10 -my-6 sm:-my-10">
                <KoreanQuiz onExit={() => setActiveTab(TABS[0])} />
              </div>
            )}
