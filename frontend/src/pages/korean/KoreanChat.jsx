...
const SCENARIOS = [
  { 
    ...
    icon: MessageSquare, 
    promptPrefix: "[Korean Language Tutor Mode] You are teaching Korean to an English speaker. For every reply, provide the Korean (Hangul) first, followed by the English transliteration in parentheses, and then the English translation. Example format: '안녕하세요! (Annyeonghaseyo!) - Hello!'. Gently correct any mistakes. User says: ",
    welcome: () => `안녕하세요! (Annyeonghaseyo!) - Hello! I am Gemma, your Korean conversation coach. You can speak in English, and I will help you translate it to Korean, or you can try speaking in Korean!`
  },
  { 
    id: "interview", 
    label: "Job Interview Roleplay", 
    icon: Briefcase, 
    promptPrefix: "[Korean Job Interview Tutor Mode] You are teaching formal Korean to an English speaker. Provide the Korean, then (transliteration), then - English translation. User says: ",
    welcome: () => `면접 연습에 오신 것을 환영합니다! (Myeonjeop yeonseube osin geoseul hwanyeonghamnida!) - Welcome to the interview practice! I will act as your hiring manager. Tell me about yourself.`
  },
  { 
    id: "business", 
    label: "Business Presentation", 
    icon: Award, 
    promptPrefix: "[Korean Business Tutor Mode] You are teaching professional Korean to an English speaker. Provide the Korean, then (transliteration), then - English translation. User says: ",
    welcome: () => `비즈니스 프레젠테이션 연습에 오신 것을 환영합니다! (Bijeuniseu peurejenteisyeon yeonseube osin geoseul hwanyeonghamnida!) - Welcome to the Business Presentation practice! What would you like to present today?`
  },
  { 
    id: "travel", 
    label: "Travel & Dining Out", 
    icon: Compass, 
    promptPrefix: "[Korean Travel Tutor Mode] You are teaching conversational travel Korean to an English speaker. Provide the Korean, then (transliteration), then - English translation. User says: ",
    welcome: () => `여행 및 외식 연습에 오신 것을 환영합니다! (Yeohaeng mit oesik yeonseube osin geoseul hwanyeonghamnida!) - Welcome to Travel & Dining Out practice! Imagine you're at a hotel in Seoul. What do you say?`
  },
];

export default function KoreanChat() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("lingolive_korean_chat_history");
      if (saved) {
  ...
  const saveSessionToHistory = (currentMsgs, scenarioId, currentId) => {
    ...
    setHistoryList((prev) => {
      ...
      try {
        localStorage.setItem("lingolive_korean_chat_history", JSON.stringify(updated));
      } catch (e) {
  ...
  const handleClearAllHistory = () => {
    if (window.confirm("Clear all chat history?")) {
      setHistoryList([]);
      localStorage.removeItem("lingolive_korean_chat_history");
    }
  };
  ...
  const handleSendMessage = async (e, customText = null) => {
    ...
    try {
      ...
      // Attaching the provided API Key per user request for Korean realtime
      const API_KEY = import.meta.env.VITE_KOREAN_REALTIME_API_KEY || "";
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-slate-400">
              Real-Time Korean Conversation Coach • Live Voice &amp; Text
            </p>
          </div>
        </div>
      ...
      <form onSubmit={(e) => handleSendMessage(e)} className="border-t border-white/10 bg-[#0f172a]/95 p-4 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            placeholder={isListening ? "Listening... speak now..." : "Type your sentence in English or Korean…"}
