    ...
    <div className={`group relative flex flex-col p-5 bg-white/80 backdrop-blur-xl rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-md overflow-hidden h-full ${
      ...
      <div className="flex-1">
        <span className="text-[22px] font-bold font-sans leading-[1.7] tracking-wide text-[#14213D] mb-3 pr-2 flex items-start gap-2 break-words">
          ...
          <span className={isLocked ? "blur-[2px] opacity-70" : ""}>{word.chinese}</span>
        </span>
  ...
  return (
    <div className={`group relative flex flex-col p-5 bg-white/80 backdrop-blur-xl rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-md overflow-hidden h-full ${
      ...
      <div className="flex justify-between items-start mb-2">
        ...
        <button 
          onClick={(e) => { e.stopPropagation(); playAudio(question.chinese); }}
          disabled={isLocked}
          className={`p-1.5 rounded-xl shadow-sm border transition-all z-10 hover:scale-110 active:scale-95 ${
      ...
      <div className="flex-1 mb-4">
        <span className="text-[22px] font-bold font-sans leading-[1.7] tracking-wide text-[#14213D] mb-3 pr-2 flex items-start gap-2 break-words">
          ...
          <span className={isLocked ? "blur-[2px] opacity-70" : ""}>{question.chinese}</span>
        </span>
...
export default function ChineseDashboard() {
  ...
  useEffect(() => {
    ...
    const defaultProgress = { initials: 0, simple_finals: 0, compound_finals: 0, words: 0, numbers: 0, sentences: 0, quiz: 0 };
  ...
  const dashboardCards = [
    { key: "initials", label: "Initials", tab: TABS[1], icon: "拼", total: alphabetData.chinese_phonetics_pinyin.initials.length, color: "#C9A227", bg: "bg-amber-50", border: "border-amber-200" },
    { key: "simple_finals", label: "Simple Finals", tab: TABS[1], icon: "音", total: alphabetData.chinese_phonetics_pinyin.simple_finals.length, color: "#3F6656", bg: "bg-emerald-50", border: "border-emerald-200" },
    { key: "compound_finals", label: "Compound Finals", tab: TABS[1], icon: "语", total: alphabetData.chinese_phonetics_pinyin.compound_finals.length, color: "#6366f1", bg: "bg-indigo-50", border: "border-indigo-200" },
    { key: "words", label: "Essential Words", tab: TABS[2], icon: "📚", total: chineseWordsData.words.length, color: "#0ea5e9", bg: "bg-sky-50", border: "border-sky-200" },
    { key: "numbers", label: "Numbers", tab: TABS[3], icon: "🔢", total: chineseNumbersData.numbers.length, color: "#ec4899", bg: "bg-pink-50", border: "border-pink-200" },
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-5">
      {lettersArray.map((letter, idx) => {
        ...
        return (
          <div 
            key={idx} 
            onClick={() => !isLocked && handleInteraction(type, idx, letter.pinyin)}
            ...
            <div className="relative z-10 grid gap-6 md:grid-cols-[1fr_auto]">
              <div className="space-y-4">
                ...
                <p className="max-w-2xl font-sans text-sm text-white/70 leading-relaxed">
                  Master the core {alphabetData.total_letters} letters of Chinese. 
              ...
              <div className="space-y-8 animate-fade-in">
                <div className="space-y-2">
                  ...
                  <p className="font-sans text-sm text-[#14213D]/60 max-w-xl">
                    Pick up where you left off or start a new lesson. Your Chinese journey is waiting for you!
                  </p>
                </div>
              ...
              <div className="space-y-12">
                <div className="space-y-6">
                  <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                    <Languages className="w-6 h-6 text-[#C9A227]" /> Initials
                  </h3>
                  {renderLetterGrid("initials", alphabetData.chinese_phonetics_pinyin.initials)}
                </div>

                <div className="space-y-6">
                  <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                    <Languages className="w-6 h-6 text-[#3F6656]" /> Simple Finals
                  </h3>
                  {renderLetterGrid("simple_finals", alphabetData.chinese_phonetics_pinyin.simple_finals)}
                </div>

                <div className="space-y-6">
                  <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                    <Languages className="w-6 h-6 text-[#6366f1]" /> Compound Finals
                  </h3>
                  {renderLetterGrid("compound_finals", alphabetData.chinese_phonetics_pinyin.compound_finals)}
                </div>
              </div>
            )}
                ...
                {activeWordPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="font-display text-xl font-bold text-[#14213D] flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-[#0ea5e9]" /> {chineseWordsData.words.length} Essential Words
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(chineseWordsData.words.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = (i + 1) * 10;
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {chineseWordsData.words.slice(activeWordPartView * 10, (activeWordPartView + 1) * 10).map((word, relIdx) => {
                         ...
                         return (
                           <WordCard 
                             ...
                             isLocked={isLocked}
                             onInteract={() => handleInteraction('words', globalIdx, word.chinese)} 
                           />
                         );
                      })}
                    </div>
            ...
            {activeTab === "Numbers" && (
              <div className="space-y-6">
                {activeNumberPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="font-display text-xl font-bold text-[#14213D] flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-[#ec4899]" /> {chineseNumbersData.numbers.length} Numbers
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(chineseNumbersData.numbers.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = (i + 1) * 10;
                        const partName = `Part ${i + 1} (${startIdx + 1}-${endIdx})`;
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {chineseNumbersData.numbers.slice(activeNumberPartView * 10, (activeNumberPartView + 1) * 10).map((number, relIdx) => {
                         ...
                         return (
                           <WordCard 
                             ...
                             isInProgress={isInProgress}
                             isLocked={isLocked}
                             onInteract={() => handleInteraction('numbers', globalIdx, number.chinese)} 
                           />
                         );
                      })}
                    </div>
            ...
            {activeTab === "Quiz" && (
              <div className="space-y-6">
                {activeQuizModuleView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="font-display text-xl font-bold text-[#14213D] flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-[#8b5cf6]" /> {chineseQuizData.total_questions} Quiz Questions
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {chineseQuizData.modules.map((moduleData, i) => (
                        <button
                          key={moduleData.module}
                          onClick={() => {
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="flex items-center justify-between">
                      <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                        <BookOpen className="w-6 h-6 text-[#8b5cf6]" /> Module {chineseQuizData.modules[activeQuizModuleView].module} ({chineseQuizData.modules[activeQuizModuleView].total_questions} Questions)
                      </h3>
                    </div>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(chineseQuizData.modules[activeQuizModuleView].quiz.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = Math.min((i + 1) * 10, chineseQuizData.modules[activeQuizModuleView].quiz.length);
                        const partName = `Part ${i + 1} (${startIdx + 1}-${endIdx})`;
                        
                        let moduleGlobalStartIdx = 0;
                        for (let m = 0; m < activeQuizModuleView; m++) {
                           moduleGlobalStartIdx += chineseQuizData.modules[m].total_questions;
                        }
                        const partGlobalStartIdx = moduleGlobalStartIdx + startIdx;
                        const partGlobalEndIdx = moduleGlobalStartIdx + endIdx - 1;
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="flex items-center justify-between">
                      <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                        <BookOpen className="w-6 h-6 text-[#8b5cf6]" /> Module {chineseQuizData.modules[activeQuizModuleView].module} - Part {activeQuizPartView + 1}
                      </h3>
                    </div>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {chineseQuizData.modules[activeQuizModuleView].quiz.slice(activeQuizPartView * 10, (activeQuizPartView + 1) * 10).map((q, relIdx) => {
                         let globalIdx = 0;
                         for (let m = 0; m < activeQuizModuleView; m++) {
                            globalIdx += chineseQuizData.modules[m].total_questions;
                         }
                         ...
                         return (
                           <InteractiveQuizCard 
                             ...
                             isLocked={isLocked}
                             playAudio={playAudio}
                             onInteract={() => handleInteraction('quiz', globalIdx, q.chinese)} 
                           />
                         );
                      })}
                    </div>
