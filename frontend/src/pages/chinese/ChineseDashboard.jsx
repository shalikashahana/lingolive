...
import chineseNumbersData from "../../data/chineseNumbersData.json";
import chineseSentencesData from "../../data/chineseSentencesData.json";
import chineseQuizData from "../../data/chineseQuizData.json";
import ChineseChat from "./ChineseChat";
import ChineseQuiz from "./ChineseQuiz";
import { useAuth } from "../../context/AuthContext";
import { 
  BookOpen, Sparkles, Languages, CheckCircle2, ChevronRight, ArrowLeft,
  Play, Volume2, Eye, EyeOff, User, LogOut, Lock, Star, Flame, Zap, BarChart3, Globe, LayoutDashboard, Search, MessageCircle, ChevronDown, ChevronUp, Clock, Target, Award, ShieldCheck
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import CatVoiceCheckpoint from "../../components/catTeacher/CatVoiceCheckpoint";

function ProgressRing({ progress, size = 44, strokeWidth = 4, color = "#EAB308" }) {
  ...
  return (
    <motion.div 
      ...
      className={`group relative flex flex-col p-5 bg-[#0f172a]/90 backdrop-blur-xl rounded-3xl border transition-all duration-300 overflow-hidden h-full shadow-lg ${
        isInProgress ? "border-amber-500 ring-2 ring-amber-500/30 bg-rose-950/20" : 
    ...
    >
      <div className="flex justify-between items-start mb-3">
        ...
        <button 
          ...
          className={`p-2 rounded-2xl border transition-all ${
            isLocked ? "bg-white/5 border-white/5 opacity-40 cursor-not-allowed" : "bg-rose-500/10 border-rose-500/30 text-rose-400 hover:bg-rose-500 hover:text-white hover:scale-110 active:scale-95"
          }`}
        >
      ...
      <div className="flex-1">
        <div className="text-2xl font-bold font-sans text-white mb-2 flex items-start gap-2 break-words">
          ...
          <span className={isLocked ? "blur-[3px] opacity-50" : "text-rose-100"}>{word.chinese || word.hanzi}</span>
        </div>
        
        <div className={`flex flex-wrap gap-2 mb-4 ${isLocked ? "opacity-40" : ""}`}>
          <span className="font-mono text-[11px] font-semibold bg-white/5 border border-white/10 text-amber-300 px-2.5 py-1 rounded-xl">
            {word.english_transliteration || word.pinyin_english || word.pinyin}
          </span>
          {(word.tamil_transliteration || word.transliteration_tamil || word.tamil) && (
...
export default function ChineseDashboard() {
  ...
  const savedTab = localStorage.getItem("chinese_active_tab");
  const [activeTab, setActiveTabState] = useState(savedTab || "Home");

  const setActiveTab = (tab) => {
    setActiveTabState(tab);
    localStorage.setItem("chinese_active_tab", tab);
  };
  ...
  useEffect(() => {
    const savedProgressStr = localStorage.getItem("chinese_progress");
    ...
    const savedStats = JSON.parse(localStorage.getItem("chinese_stats") || '{"streak":0,"xp":0}');
    setStats(savedStats);
  }, []);
  ...
  const formattedChineseSentences = useMemo(() => {
    const parts = [];
    if (Array.isArray(chineseSentencesData)) {
      const sentences = chineseSentencesData;
      for (let i = 0; i < sentences.length; i += 10) {
        ...
        const levelIndex = Math.floor(indexInModule / 100);
        ...
        parts.push({
          ...
          levelName: levelNum,
          phase: `Module ${moduleNum} - Level ${levelNum} - Part ${Math.floor(indexInModule / 10) + 1} (${indexInModule + 1}-${indexInModule + partSentences.length})`,
          context: "Basic Sentences",
          sentences: partSentences.map(s => ({
            english: s.meaning_english || s.english_meaning || "",
            tamil: s.meaning_tamil || s.tamil_meaning || "",
            chinese: s.chinese || "",
            transliteration: s.pinyin_english || s.english_transliteration || ""
          }))
        });
      }
    } else if (chineseSentencesData.modules) {
      chineseSentencesData.modules.forEach((mod, modIndex) => {
  ...
  const [completedPhases, setCompletedPhases] = useState(() => {
    const saved = localStorage.getItem("chinese_completed_phases");
  ...
  const handleCheckpointComplete = (score, total) => {
    if (checkpointPhase) {
      ...
      localStorage.setItem("chinese_completed_phases", JSON.stringify(updated));
    }
  };
  ...
  }, [searchQuery, formattedChineseSentences, activeSentenceModuleView, activeSentenceLevelView]);
  ...
  // Chinese Imperial Red + Gold Visual Theme Cards
  const dashboardCards = [
    { key: "swarangal", label: "Initials (声母)", native: "声母", tab: TABS[1], icon: "b", total: (alphabetData.chinese_phonetics_pinyin?.initials || []).length, color: "#BE123C", estimatedTime: "20 mins", difficulty: "Beginner" },
    { key: "vyanjanangal", label: "Finals (韵母)", native: "韵母", tab: TABS[1], icon: "a", total: (alphabetData.chinese_phonetics_pinyin?.simple_finals || []).length, color: "#EAB308", estimatedTime: "25 mins", difficulty: "Beginner" },
    { key: "words", label: "Essential Words", native: "词汇", tab: TABS[2], icon: "📚", total: (chineseWordsData || []).length, color: "#0EA5E9", estimatedTime: "45 mins", difficulty: "Beginner" },
    { key: "numbers", label: "Numbers", native: "数字", tab: TABS[3], icon: "🔢", total: (chineseNumbersData?.chinese_numbers_1_to_100 || []).length, color: "#EC4899", estimatedTime: "25 mins", difficulty: "Beginner" },
    { key: "sentences", label: "Sentences", native: "句子", tab: TABS[4], icon: "💬", total: chineseSentencesData.total_sentences || chineseSentencesData.length || 0, color: "#10B981", estimatedTime: "60 mins", difficulty: "Intermediate" },
    { key: "quiz", label: "Quiz Dashboard", native: "测验", tab: TABS[5], icon: "🧠", total: chineseQuizData.total_questions, color: "#8B5CF6", estimatedTime: "20 mins", difficulty: "All Levels" }
  ];
      ...
      {lettersArray.map((letter, idx) => {
        ...
        return (
          <motion.div 
            ...
            onClick={() => !isLocked && handleInteraction(type, idx, letter.pinyin || letter.letter)}
          ...
          >
            ...
            <div className="my-1 flex h-12 w-12 items-center justify-center rounded-2xl font-mono text-base font-bold bg-white/5 text-amber-300 border border-white/10 shadow-sm group-hover:border-amber-500/40 transition-colors">
              <span className={`text-3xl font-bold font-sans ${isLocked ? 'text-slate-600' : 'text-amber-200 group-hover:text-amber-400'} transition-colors`}>
                {letter.pinyin || letter.letter}
              </span>
            </div>
            
            <div className="flex flex-col items-center gap-1 z-10 w-full px-1">
              <span className="font-mono text-[10px] font-semibold bg-white/5 text-amber-300 px-1.5 py-0.5 rounded-lg w-full text-center truncate border border-white/5">
                {letter.approximate_sound || letter.transliteration}
              </span>
            </div>
          </motion.div>
        );
  ...
  return (
    <div className="flex h-screen bg-[#050816] overflow-hidden font-sans text-white">
      {/* Sidebar */}
      <aside className="w-64 h-screen bg-[#090d1f]/95 border-r border-rose-500/10 flex flex-col backdrop-blur-2xl shrink-0">
        <div className="p-5 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-tr from-rose-700 to-amber-400 text-white shadow-lg shadow-rose-600/20 font-bold">
                ...
                {availableLanguages.map((lang) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 px-4 py-2.5 font-sans text-sm font-semibold transition-colors ${
                      currentLanguageCode === lang.code ? "bg-rose-500/10 text-amber-400" : "text-slate-300 hover:bg-white/5"
                    }`}
                  >
                    <span>{lang.flag}</span>
                    <span>{lang.name}</span>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-1 custom-scrollbar">
          {TABS.map((tabName) => {
            const getIcon = (name) => {
              ...
              if (name.includes("Pinyin")) return <span className="text-sm font-sans font-bold">中</span>;
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-2xl font-semibold text-sm transition-all duration-200 ${
                  isActive
                    ? "bg-gradient-to-r from-rose-700/30 to-amber-500/10 border border-rose-500/40 text-amber-300 shadow-sm"
              ...
              >
                ...
                {isActive && <div className="h-2 w-2 rounded-full bg-amber-400 shadow-[0_0_8px_#eab308] shrink-0" />}
              </button>
            );
          })}
        </nav>
        ...
        <div className="p-4 border-t border-white/10 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="flex items-center gap-2 rounded-xl bg-rose-500/10 border border-rose-500/20 px-3 py-2">
              <Zap className={`w-4 h-4 ${stats.xp > 0 ? "text-rose-400 fill-rose-400" : "text-slate-500"}`} />
              <div>
                <div className="font-bold text-sm text-rose-300 leading-none">{stats.xp}</div>
                <div className="text-[9px] font-mono text-rose-600 uppercase tracking-wider mt-0.5">XP</div>
              </div>
            </div>
          </div>
          ...
          {user && (
            <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-2.5">
              <div className="flex items-center gap-2.5 overflow-hidden">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-tr from-rose-700 to-amber-400 text-xs font-bold text-white">
                  {user.displayName?.[0] || user.email?.[0]?.toUpperCase() || "C"}
                </div>
                <div className="overflow-hidden">
                  <p className="text-xs font-bold text-white truncate">{user.displayName || "Chinese Learner"}</p>
        ...
        <div className="p-6 sm:p-8 max-w-6xl mx-auto w-full space-y-8 pb-16">
          ...
          {activeTab === "Home" && (
            <motion.div
              ...
              className="relative overflow-hidden rounded-3xl border border-amber-500/30 bg-gradient-to-r from-rose-950/40 via-[#0f172a] to-[#050816] p-8 sm:p-10 shadow-2xl"
            >
              <div className="absolute -right-24 -top-24 h-72 w-72 rounded-full bg-rose-600/15 blur-3xl pointer-events-none" />
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                <div className="space-y-3">
                  <div className="inline-flex items-center gap-2 rounded-full border border-amber-400/30 bg-amber-400/10 px-3.5 py-1 text-xs font-mono font-bold text-amber-300 backdrop-blur-md">
                    ...
                    Mandarin Pinyin · Imperial Style
                  </div>
                  ...
                  <p className="max-w-xl text-sm sm:text-base text-slate-400 leading-relaxed">
                    Master Mandarin Initials (声母), Finals (韵母), essential Chinese characters (汉字), and daily sentence tones.
                  </p>
                </div>
                
                <div className="flex items-center gap-4 bg-white/5 border border-white/10 p-4 rounded-3xl backdrop-blur-md shrink-0">
                  <ProgressRing progress={Math.min(100, ((progress.swarangal + progress.vyanjanangal + progress.words) / 80) * 100)} size={56} strokeWidth={5} color="#EAB308" />
                  <div>
                    <div className="text-xs font-mono font-bold text-slate-400 uppercase tracking-wider">Overall Goal</div>
                    <div className="text-lg font-extrabold text-white">Mandarin Mastery</div>
                    <div className="text-[11px] text-amber-400 font-medium">Daily Chinese practice</div>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          <div className="space-y-6">
            {activeTab === "Home" && (
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  {/* Streak Metric */}
                  <div className="p-5 border border-amber-500/20 rounded-2xl bg-[#0f172a]/80 flex items-center justify-between shadow-lg">
                    <div>
                      <span className="font-mono text-[11px] font-bold text-amber-500/70 uppercase tracking-wider block mb-1">Daily Streak</span>
                  ...
                  {/* XP Progress Metric */}
                  <div className="p-5 border border-rose-500/20 rounded-2xl bg-[#0f172a]/80 shadow-lg">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-mono text-[11px] font-bold text-rose-500/70 uppercase tracking-wider">Level {Math.floor(stats.xp / 100) + 1} XP</span>
                      <span className="font-number text-xs font-bold text-rose-400">{stats.xp} / {(Math.floor(stats.xp / 100) + 1) * 100}</span>
                    </div>
                    <div className="w-full h-2.5 bg-white/5 rounded-full overflow-hidden mb-1">
                      <div
                        ...
                        style={{ width: `${Math.min(100, (stats.xp / ((Math.floor(stats.xp / 100) + 1) * 100)) * 100)}%` }}
                      />
                    </div>
                    <span className="text-[10px] text-slate-400 font-medium">
                      {Math.max(0, ((Math.floor(stats.xp / 100) + 1) * 100) - stats.xp)} XP remaining to Level {Math.floor(stats.xp / 100) + 2}
                    </span>
                  </div>

                  {/* Interview Readiness */}
                  <div className="p-5 border border-emerald-500/20 rounded-2xl bg-[#0f172a]/80 flex items-center justify-between shadow-lg">
                    <div>
                      <span className="font-mono text-[11px] font-bold text-emerald-500/70 uppercase tracking-wider block mb-1">Interview Readiness</span>
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-slate-300">Not Available</span>
                  ...
                  {/* Vocabulary Progress */}
                  <div className="p-5 border border-sky-500/20 rounded-2xl bg-[#0f172a]/80 flex items-center justify-between shadow-lg">
                    <div>
                      <span className="font-mono text-[11px] font-bold text-sky-500/70 uppercase tracking-wider block mb-1">Vocabulary Progress</span>
                      <div className="flex items-baseline gap-1.5">
                        <span className="font-number text-2xl font-extrabold text-sky-400">{progress.words}</span>
                        <span className="text-xs text-slate-400 font-semibold">/ {chineseWordsData.length} Words</span>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 p-6 rounded-3xl border border-amber-500/20 bg-gradient-to-br from-[#0f172a] to-[#090d1f] flex flex-col justify-between space-y-4 shadow-xl">
                    ...
                    <div className="space-y-3">
                      {[
                        { text: "Learn 5 Pinyin Initials (声母)", tab: "Pinyin & Finals (拼音)", done: progress.swarangal >= 5 },
                        { text: "Study 10 Chinese Vocabulary Words", tab: "Essential Words", done: progress.words >= 10 },
                        { text: "Take Mandarin Practice Quiz", tab: "Quiz Dashboard", done: false },
                      ].map((item, i) => (
                        <div
                          key={i}
                  ...
                  <div className="p-6 rounded-3xl border border-rose-500/20 bg-gradient-to-b from-rose-950/30 via-[#0f172a] to-[#050816] flex flex-col justify-between space-y-4 shadow-xl">
                    <div>
                      <div className="h-10 w-10 rounded-2xl bg-gradient-to-tr from-rose-700 to-amber-400 flex items-center justify-center text-white shadow-md mb-3 font-bold">
                        <Sparkles className="w-5 h-5" />
                      </div>
                      <h3 className="text-lg font-bold text-white mb-1">Continue Learning</h3>
                      <p className="text-xs text-slate-400 leading-relaxed">
                        Keep refining your Mandarin pronunciation and tones.
                      </p>
                    </div>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-5">
                    <h2 className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">Chinese Modules</h2>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-6 w-full">
                            <div className="h-12 w-12 rounded-2xl flex items-center justify-center text-xl font-bold text-white shadow-md bg-rose-500/20 border border-rose-500/30">
                              {card.icon}
                            </div>
                          ...
                          <div className="w-full space-y-2">
                            ...
                            <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
                              <div
                                className="h-full rounded-full bg-gradient-to-r from-rose-700 to-amber-400 transition-all duration-700"
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          </div>
            ...
            {activeTab === "Pinyin & Finals (拼音)" && (
              <div className="space-y-10">
                <div className="space-y-4">
                  <h3 className="text-2xl font-bold text-white flex items-center gap-2">
                    <Languages className="w-6 h-6 text-rose-400" /> Initials (声母)
                  </h3>
                  {renderLetterGrid("swarangal", alphabetData.chinese_phonetics_pinyin.initials)}
                </div>

                <div className="space-y-4">
                  <h3 className="text-2xl font-bold text-white flex items-center gap-2">
                    <Languages className="w-6 h-6 text-amber-400" /> Finals (韵母)
                  </h3>
                  {renderLetterGrid("vyanjanangal", alphabetData.chinese_phonetics_pinyin.simple_finals)}
                </div>
              </div>
            )}
                ...
                {activeWordPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-sky-400" /> {chineseWordsData.length} Essential Chinese Words
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(chineseWordsData.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = (i + 1) * 10;
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {chineseWordsData.slice(activeWordPartView * 10, (activeWordPartView + 1) * 10).map((word, relIdx) => {
                         ...
                         return (
                           <WordCard 
                             ...
                             isLocked={isLocked}
                             onInteract={() => handleInteraction('words', globalIdx, word.chinese || word.hanzi)} 
            ...
            {activeTab === "Numbers" && (
              <div className="space-y-6">
                {activeNumberPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-pink-400" /> {chineseNumbersData.chinese_numbers_1_to_100.length} Chinese Numbers
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(chineseNumbersData.chinese_numbers_1_to_100.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = (i + 1) * 10;
                        const partName = `Part ${i + 1} (${startIdx + 1}-${endIdx})`;
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {chineseNumbersData.chinese_numbers_1_to_100.slice(activeNumberPartView * 10, (activeNumberPartView + 1) * 10).map((number, relIdx) => {
                         ...
                         return (
                           <WordCard 
                             ...
                             isInProgress={isInProgress}
                             isLocked={isLocked}
                             onInteract={() => handleInteraction('numbers', globalIdx, number.chinese || number.hanzi)} 
                           />
                         );
                      })}
                    </div>
            ...
            {activeTab === "Sentences" && (
              <div className="space-y-8 pb-16 w-full max-w-4xl mx-auto">
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-rose-950/40 via-[#0f172a] to-[#050816] p-8 border border-amber-500/30 shadow-2xl">
                  <h1 className="text-3xl font-extrabold text-white mb-2">Daily Conversations (句子)</h1>
                  <p className="text-sm text-slate-400">Master everyday Chinese sentences grouped by practical scenarios.</p>
                </div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Search sentences in Chinese, English or Tamil..."
                ...
                <div className="space-y-4">
                  {activeSentenceModuleView === null && !searchQuery.trim() ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from(new Set(formattedChineseSentences.map(p => p.moduleIndex))).map((moduleIdx) => {
                        const moduleData = formattedChineseSentences.filter(p => p.moduleIndex === moduleIdx);
                                ...
                                return (
                                  <div key={sIndex} className="p-5 rounded-2xl bg-[#0f172a]/80 backdrop-blur-sm border border-white/10 hover:border-amber-500/30 transition-all shadow-lg flex flex-col justify-between min-h-[140px]">
                                    <div className="flex items-start justify-between mb-4">
                                      <p className="text-xl font-bold text-amber-200 leading-relaxed pr-4">{sentence.chinese}</p>
                                      <button onClick={() => playAudio(sentence.chinese)} className="p-2.5 rounded-xl bg-rose-500/10 text-amber-400 hover:bg-rose-500 hover:text-white transition-all shrink-0">
                                        <Volume2 className="h-4 w-4" />
                                      </button>
                                    </div>
                                    <div>
                                      <button onClick={() => toggleTranslation(sIndex, data.phase)} className="text-xs font-bold text-amber-400 hover:text-amber-300 transition-colors">
            ...
            {activeTab === "Quiz" && (
              <div className="-mx-4 sm:-mx-10 -my-6 sm:-my-10">
                <ChineseQuiz onExit={() => setActiveTab(TABS[0])} />
              </div>
            )}
