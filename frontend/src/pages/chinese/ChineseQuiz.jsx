...
import chineseQuizJson from "../../data/chineseQuizData.json";
...
export default function ChineseQuiz({ onExit }) {
  const navigate = useNavigate();
  const { triggerCatTeacherModal } = useCatTeacher();
  const allQuestions = Array.isArray(chineseQuizJson) ? chineseQuizJson : Object.values(chineseQuizJson).flat();
  ...
  useEffect(() => {
    const savedLevel = localStorage.getItem("chinese_quiz_unlocked_level");
    if (savedLevel) {
  ...
  const handleMCQSubmit = (idx, isCorrect, zhText) => {
    if (isAnswered) return;
    setSelectedOption(idx);
    setAnswersSubmitted((prev) => ({ ...prev, [currentQ.id]: isCorrect }));
    
    if (zhText) playAudio(zhText);
      ...
      triggerCatTeacherModal({
        ...
        onUnlockNextLevel: (nextLvl) => {
          if (activeLevel === unlockedLevel && activeLevel < totalLevels) {
            ...
            localStorage.setItem("chinese_quiz_unlocked_level", nextLvl.toString());
          }
          ...
          localStorage.setItem("chinese_stats", JSON.stringify({
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-display text-3xl font-bold text-white flex items-center gap-2">
              <Zap className="w-8 h-8 text-red-400" /> Chinese Quiz Modules
            </h2>
          ...
          {currentQ && (
            <div className="space-y-3">
              ...
              <h2 className="font-display text-2xl sm:text-3xl font-bold text-white leading-relaxed">
                {currentQ.question || `What is the Chinese word for "${currentQ.english}" (${currentQ.tamil})?`}
              </h2>
            </div>
          )}
          ...
          {currentQ && (
            <div className="space-y-3.5">
              {currentQ.options.map((opt, idx) => {
                ...
                const isCorrect = `${opt.hanzi} (${opt.pinyin})` === currentQ.correct_answer;
                ...
                return (
                  <button
                    key={idx}
                    disabled={isAnswered}
                    onClick={() => handleMCQSubmit(idx, isCorrect, opt.hanzi)}
                  ...
                  >
                    <div className="flex flex-col gap-0.5">
                      <span className="font-bold font-sans text-2xl tracking-wide">{opt.hanzi}</span>
                      <span className={`text-xs font-mono mt-0.5 ${isAnswered && isCorrect ? 'text-emerald-400' : isAnswered && isSelected && !isCorrect ? 'text-red-400' : 'text-slate-400'}`}>{opt.pinyin}</span>
                    </div>
