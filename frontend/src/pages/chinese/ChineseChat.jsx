...
const SCENARIOS = [
  { 
    ...
    icon: MessageSquare, 
    promptPrefix: "[Chinese Language Tutor Mode] You are teaching Chinese to an English speaker. For every reply, provide the Chinese (Characters/Pinyin) first, followed by the English transliteration in parentheses, and then the English translation. Example format: '你好！(Nǐ hǎo!) - Hello!'. Gently correct any mistakes. User says: ",
    welcome: () => `你好！(Nǐ hǎo!) - Hello! I am Gemma, your Chinese conversation coach. You can speak in English, and I will help you translate it to Chinese, or you can try speaking in Chinese!`
  },
  { 
    id: "interview", 
    label: "Job Interview Roleplay", 
    icon: Briefcase, 
    promptPrefix: "[Chinese Job Interview Tutor Mode] You are teaching formal Chinese to an English speaker. Provide the Chinese, then (transliteration), then - English translation. User says: ",
    welcome: () => `欢迎来到面试练习！(Huānyíng lái dào miànshì liànxí!) - Welcome to the interview practice! I will act as your hiring manager. Tell me about yourself.`
  },
  { 
    id: "business", 
    label: "Business Presentation", 
    icon: Award, 
    promptPrefix: "[Chinese Business Tutor Mode] You are teaching professional Chinese to an English speaker. Provide the Chinese, then (transliteration), then - English translation. User says: ",
    welcome: () => `欢迎来到商务演讲练习！(Huānyíng lái dào shāngwù yǎnjiǎng liànxí!) - Welcome to the Business Presentation practice! What would you like to present today?`
  },
  { 
    id: "travel", 
    label: "Travel & Dining Out", 
    icon: Compass, 
    promptPrefix: "[Chinese Travel Tutor Mode] You are teaching conversational travel Chinese to an English speaker. Provide the Chinese, then (transliteration), then - English translation. User says: ",
    welcome: () => `欢迎来到旅游和外出就餐练习！(Huānyíng lái dào lǚyóu hé wàichū jiùcān liànxí!) - Welcome to Travel & Dining Out practice! Imagine you're at a hotel in Beijing. What do you say?`
  },
];

export default function ChineseChat() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("lingolive_chinese_chat_history");
      if (saved) {
  ...
  const saveSessionToHistory = (currentMsgs, scenarioId, currentId) => {
    ...
    setHistoryList((prev) => {
      ...
      try {
        localStorage.setItem("lingolive_chinese_chat_history", JSON.stringify(updated));
      } catch (e) {
  ...
  const handleClearAllHistory = () => {
    if (window.confirm("Clear all chat history?")) {
      setHistoryList([]);
      localStorage.removeItem("lingolive_chinese_chat_history");
    }
  };
  ...
  const handleSendMessage = async (e, customText = null) => {
    ...
    try {
      ...
      // Attaching the provided API Key per user request for Chinese realtime
      const API_KEY = import.meta.env.VITE_CHINESE_REALTIME_API_KEY || "";
  ...
  return (
    <div className="relative mx-auto max-w-5xl h-[calc(100vh-7.5rem)] flex flex-col rounded-3xl border border-white/10 bg-[#050816] text-white shadow-2xl overflow-hidden font-sans">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 bg-gradient-to-r from-rose-950/80 via-[#0f172a] to-[#050816] px-6 py-4 backdrop-blur-xl z-10">
        <div className="flex items-center gap-3.5">
          <div className="relative">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-tr from-rose-600 to-amber-400 text-white shadow-lg shadow-rose-500/30">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-slate-400">
              Real-Time Chinese Conversation Coach • Live Voice &amp; Text
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-amber-400/30 bg-amber-500/10 px-3.5 py-1.5 text-xs font-bold text-amber-300 hover:bg-amber-500/20 transition-all shadow-sm"
            title="Open Chat History"
          >
            <History className="h-4 w-4 text-amber-400" />
            <span>History</span>
            {historyList.length > 0 && (
              <span className="ml-1 rounded-full bg-amber-400 text-[#050816] px-1.5 py-0.2 font-mono text-[10px] font-extrabold">
                {historyList.length}
              </span>
            )}
          </button>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 font-medium transition-all shrink-0 ${
                  isSelected
                    ? "bg-rose-600/30 border border-rose-500/40 text-amber-300 shadow-sm"
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 custom-scrollbar bg-radial-gradient">
            ...
            >
              ...
              <div
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl font-bold shadow-md ${
                  msg.sender === "user"
                    ? "bg-gradient-to-tr from-amber-600 to-rose-500 text-white"
                    : "bg-gradient-to-tr from-rose-900 to-slate-800 border border-white/10 text-amber-400"
                }`}
              >
        ...
        {loading && (
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-rose-900/50 text-amber-400 border border-white/10">
      ...
      <div className="border-t border-white/5 bg-[#050816]/90 px-6 py-2 flex items-center gap-2 overflow-x-auto custom-scrollbar">
        ...
        {suggestionChips.map((chip, idx) => (
          <button
            ...
            disabled={loading}
            className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] text-slate-300 hover:border-amber-400/40 hover:bg-white/10 hover:text-white transition-all shrink-0"
          >
            {chip}
          </button>
        ))}
      ...
      <form onSubmit={(e) => handleSendMessage(e)} className="border-t border-white/10 bg-[#0f172a]/95 p-4 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            placeholder={isListening ? "Listening... speak now..." : "Type your sentence in English or Chinese…"}
          ...
          <button
            ...
            className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-r from-rose-600 to-amber-500 text-white shadow-lg shadow-rose-600/30 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 shrink-0"
          >
          ...
          >
            ...
            >
              <div>
                ...
                <button
                  onClick={handleStartNewChat}
                  className="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-rose-600 to-amber-500 py-3 font-sans text-sm font-bold text-white shadow-lg shadow-rose-600/30 hover:scale-[1.02] transition-all mb-6"
                >
                    ...
                    historyList.map((item) => (
                      ...
                      >
                        <div className="flex items-center gap-3 overflow-hidden">
                          <div className="h-9 w-9 rounded-xl bg-rose-600/20 text-amber-400 flex items-center justify-center shrink-0">
                            <MessageCircle className="h-4 w-4" />
                          </div>
                          <div className="overflow-hidden text-left">
                            <span className="block font-mono text-[10px] font-bold text-amber-400 uppercase tracking-wider">
                              {item.scenarioLabel}
                            </span>
