...
import ArabicChat from "./ArabicChat";

function ProgressRing({ progress, size = 44, strokeWidth = 4, color = "#D97706" }) {
  ...
  return (
    <motion.div 
      ...
      className={`group relative flex flex-col p-5 bg-[#0f172a]/90 backdrop-blur-xl rounded-3xl border transition-all duration-300 overflow-hidden h-full shadow-lg ${
        isInProgress ? "border-amber-500 ring-2 ring-amber-500/30 bg-emerald-950/20" : 
    ...
    >
      <div className="flex justify-between items-start mb-3">
        ...
        <button 
          ...
          className={`p-2 rounded-2xl border transition-all ${
            isLocked ? "bg-white/5 border-white/5 opacity-40 cursor-not-allowed" : "bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500 hover:text-white hover:scale-110 active:scale-95"
          }`}
        >
      ...
      <div className="flex-1">
        <div className="text-3xl font-bold font-sans text-white mb-2 flex items-start gap-2 break-words justify-end text-right">
          {word.digit && (
          ...
          <span className={isLocked ? "blur-[3px] opacity-50" : "text-amber-100"}>{word.arabic}</span>
        </div>
        
        <div className={`flex flex-wrap gap-2 mb-4 justify-end ${isLocked ? "opacity-40" : ""}`}>
...
export default function ArabicDashboard() {
  ...
  const formattedArabicSentences = useMemo(() => {
    const parts = [];
    arabicSentencesData.modules.forEach((mod, modIndex) => {
  ...
  const [completedPhases, setCompletedPhases] = useState(() => {
    const saved = localStorage.getItem("arabic_completed_phases");
  ...
  const handleCheckpointComplete = (score, total) => {
    if (checkpointPhase) {
      ...
      localStorage.setItem("arabic_completed_phases", JSON.stringify(updated));
    }
  };

  const filteredData = useMemo(() => {
    let dataToFilter = formattedArabicSentences;
    if (!searchQuery.trim()) {
    ...
    return dataToFilter.map(phaseObj => {
      const matchingSentences = phaseObj.sentences.filter(s => {
        ...
        const arMatch = s.arabic.includes(searchQuery);
        return enMatch || taMatch || arMatch;
      });
      return { ...phaseObj, sentences: matchingSentences };
    }).filter(phaseObj => phaseObj.sentences.length > 0);
  }, [searchQuery, formattedArabicSentences, activeSentenceModuleView, activeSentenceLevelView]);
  ...
  // Arabic Dark Emerald + Metallic Gold Visual Theme Cards
  const dashboardCards = [
    { key: "swarangal", label: "Letters (حروف)", native: "حروف", tab: TABS[1], icon: "أ", total: alphabetData.alphabet.letters.length, color: "#D97706", estimatedTime: "20 mins", difficulty: "Beginner" },
    { key: "words", label: "Essential Words", native: "كلمات", tab: TABS[2], icon: "📚", total: arabicWordsData.words.length, color: "#0EA5E9", estimatedTime: "45 mins", difficulty: "Beginner" },
    { key: "numbers", label: "Numbers", native: "أرقام", tab: TABS[3], icon: "🔢", total: arabicNumbersData.numbers.length, color: "#EC4899", estimatedTime: "25 mins", difficulty: "Beginner" },
    { key: "sentences", label: "Sentences", native: "جمل", tab: TABS[4], icon: "💬", total: arabicSentencesData.total_sentences, color: "#10B981", estimatedTime: "60 mins", difficulty: "Intermediate" },
    { key: "quiz", label: "Quiz Dashboard", native: "اختبار", tab: TABS[5], icon: "🧠", total: arabicQuizData.total_questions, color: "#8B5CF6", estimatedTime: "20 mins", difficulty: "All Levels" }
  ];
  ...
  return (
    <div className="flex h-screen bg-[#050816] overflow-hidden font-sans text-white">
      {/* Sidebar */}
      <aside className="w-64 h-screen bg-[#090d1f]/95 border-r border-emerald-500/10 flex flex-col backdrop-blur-2xl shrink-0">
        <div className="p-5 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-tr from-emerald-700 to-amber-400 text-white shadow-lg shadow-emerald-600/20 font-bold">
              <Zap className="h-5 w-5 fill-white" />
            </div>
            <div>
              <span className="font-extrabold text-white tracking-tight leading-none block text-base">LingoLive</span>
              <span className="text-[10px] font-mono font-bold text-amber-400 uppercase tracking-widest">Arabic Jewels</span>
            </div>
          </div>
                ...
                {availableLanguages.map((lang) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 px-4 py-2.5 font-sans text-sm font-semibold transition-colors ${
                      currentLanguageCode === lang.code ? "bg-emerald-500/10 text-amber-400" : "text-slate-300 hover:bg-white/5"
                    }`}
                  >
                    <span>{lang.flag}</span>
                    <span>{lang.name}</span>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-1 custom-scrollbar">
          {TABS.map((tabName) => {
            const getIcon = (name) => {
              ...
              if (name.includes("Alphabets")) return <span className="text-sm font-sans font-bold">أ</span>;
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-2xl font-semibold text-sm transition-all duration-200 ${
                  isActive
                    ? "bg-gradient-to-r from-emerald-800/30 to-amber-500/10 border border-emerald-500/40 text-amber-300 shadow-sm"
              ...
              >
                ...
                {isActive && <div className="h-2 w-2 rounded-full bg-amber-400 shadow-[0_0_8px_#d97706] shrink-0" />}
              </button>
            );
          })}
        </nav>
        ...
        <div className="p-4 border-t border-white/10 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="flex items-center gap-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-3 py-2">
              <Zap className={`w-4 h-4 ${stats.xp > 0 ? "text-emerald-400 fill-emerald-400" : "text-slate-500"}`} />
              <div>
                <div className="font-bold text-sm text-emerald-300 leading-none">{stats.xp}</div>
                <div className="text-[9px] font-mono text-emerald-600 uppercase tracking-wider mt-0.5">XP</div>
              </div>
            </div>
          </div>
          ...
          {user && (
            <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-2.5">
              <div className="flex items-center gap-2.5 overflow-hidden">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-tr from-emerald-700 to-amber-400 text-xs font-bold text-white">
                  {user.displayName?.[0] || user.email?.[0]?.toUpperCase() || "A"}
                </div>
                <div className="overflow-hidden">
                  <p className="text-xs font-bold text-white truncate">{user.displayName || "Arabic Learner"}</p>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto bg-[#050816] custom-scrollbar">
        <div className="p-6 sm:p-8 max-w-6xl mx-auto w-full space-y-8 pb-16">
          ...
          <motion.div
            ...
            className="relative overflow-hidden rounded-3xl border border-emerald-500/30 bg-gradient-to-r from-emerald-950/40 via-[#0f172a] to-[#050816] p-8 sm:p-10 shadow-2xl"
          >
            <div className="absolute -right-24 -top-24 h-72 w-72 rounded-full bg-emerald-600/15 blur-3xl pointer-events-none" />
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="space-y-3">
                <div className="inline-flex items-center gap-2 rounded-full border border-amber-400/30 bg-amber-400/10 px-3.5 py-1 text-xs font-mono font-bold text-amber-300 backdrop-blur-md">
                  ...
                  Arabic Abjad · Emerald Luxury Style
                </div>
                ...
                <p className="max-w-xl text-sm sm:text-base text-slate-400 leading-relaxed">
                  Master Arabic alphabet letters (أبجدية), vocabulary words, numerals, and natural speaking expressions.
                </p>
              </div>
              
              <div className="flex items-center gap-4 bg-white/5 border border-white/10 p-4 rounded-3xl backdrop-blur-md shrink-0">
                <ProgressRing progress={Math.min(100, ((progress.swarangal + progress.words) / 80) * 100)} size={56} strokeWidth={5} color="#D97706" />
                <div>
                  <div className="text-xs font-mono font-bold text-slate-400 uppercase tracking-wider">Overall Goal</div>
                  <div className="text-lg font-extrabold text-white">Arabic Mastery</div>
                  <div className="text-[11px] text-amber-400 font-medium">Daily Arabic practice</div>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-6">
            {activeTab === "Home" && (
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 p-6 rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-[#0f172a] to-[#090d1f] flex flex-col justify-between space-y-4 shadow-xl">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        <div className="h-9 w-9 rounded-xl bg-emerald-500/20 flex items-center justify-center text-emerald-400 border border-emerald-500/30">
                    ...
                    <div className="space-y-3">
                      {[
                        { text: "Learn 5 Arabic Alphabet Letters (أبجدية)", tab: "Alphabets (أبجدية)", done: progress.swarangal >= 5 },
                        { text: "Study 10 Arabic Vocabulary Words", tab: "Essential Words", done: progress.words >= 10 },
                        { text: "Take Arabic Skill Practice Quiz", tab: "Quiz Dashboard", done: false },
                      ].map((item, i) => (
                        <div
                          key={i}
                  ...
                  <div className="p-6 rounded-3xl border border-amber-500/20 bg-gradient-to-b from-amber-950/30 via-[#0f172a] to-[#050816] flex flex-col justify-between space-y-4 shadow-xl">
                    <div>
                      <div className="h-10 w-10 rounded-2xl bg-gradient-to-tr from-emerald-700 to-amber-400 flex items-center justify-center text-white shadow-md mb-3 font-bold">
                        <Sparkles className="w-5 h-5" />
                      </div>
                      <h3 className="text-lg font-bold text-white mb-1">Continue Learning</h3>
                      <p className="text-xs text-slate-400 leading-relaxed">
                        Keep building Arabic pronunciation and word reading skills.
                      </p>
                    </div>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-5">
                    <h2 className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">Arabic Modules</h2>
                    ...
                    {dashboardCards.map((card) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className="group relative text-left p-6 rounded-3xl border border-white/10 bg-[#0f172a]/90 flex flex-col justify-between overflow-hidden transition-all duration-300 hover:border-emerald-500/40 hover:shadow-2xl shadow-lg"
                        >
                          <div className="flex items-center justify-between mb-6 w-full">
                            <div className="h-12 w-12 rounded-2xl flex items-center justify-center text-xl font-bold text-white shadow-md bg-emerald-500/20 border border-emerald-500/30">
                              {card.icon}
                            </div>
                          ...
                          <div className="w-full space-y-2">
                            ...
                            <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
                              <div
                                className="h-full rounded-full bg-gradient-to-r from-emerald-700 to-amber-400 transition-all duration-700"
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          </div>
            ...
            {activeTab === "Alphabets (أبجدية)" && (
              <div className="space-y-10">
                <div className="space-y-4">
                  <h3 className="text-2xl font-bold text-white flex items-center gap-2">
                    <Languages className="w-6 h-6 text-amber-400" /> Arabic Alphabet (أبجدية)
                  </h3>
                  {renderLetterGrid("swarangal", alphabetData.alphabet.letters)}
                </div>
              </div>
            )}
                ...
                {activeWordPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-sky-400" /> {arabicWordsData.words.length} Essential Arabic Words
                    </h3>
            ...
            {activeTab === "Numbers" && (
              <div className="space-y-6">
                {activeNumberPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-pink-400" /> {arabicNumbersData.numbers.length} Arabic Numbers
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            ...
            {activeTab === "Sentences" && (
              <div className="space-y-8 pb-16 w-full max-w-4xl mx-auto">
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-950/40 via-[#0f172a] to-[#050816] p-8 border border-emerald-500/30 shadow-2xl">
                  <h1 className="text-3xl font-extrabold text-white mb-2">Daily Conversations (جمل)</h1>
                  <p className="text-sm text-slate-400">Master everyday Arabic sentences grouped by practical scenarios.</p>
                </div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Search sentences in Arabic, English or Tamil..."
                ...
                <div className="space-y-4">
                  {activeSentenceModuleView === null && !searchQuery.trim() ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from(new Set(formattedArabicSentences.map(p => p.moduleIndex))).map((moduleIdx) => {
                        const moduleData = formattedArabicSentences.filter(p => p.moduleIndex === moduleIdx);
                                ...
                                return (
                                  <div key={sIndex} className="p-5 rounded-2xl bg-[#0f172a]/80 backdrop-blur-sm border border-white/10 hover:border-amber-500/30 transition-all shadow-lg flex flex-col justify-between min-h-[140px]">
                                    <div className="flex items-start justify-between mb-4">
                                      <p className="text-2xl font-bold text-amber-200 text-right w-full pr-4 leading-relaxed">{sentence.arabic}</p>
                                      <button onClick={() => playAudio(sentence.arabic)} className="p-2.5 rounded-xl bg-amber-500/10 text-amber-400 hover:bg-amber-500 hover:text-white transition-all shrink-0">
