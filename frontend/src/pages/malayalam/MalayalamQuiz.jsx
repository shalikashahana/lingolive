...
export default function MalayalamQuiz({ onExit }) {
      ...
      <div className="mx-auto max-w-4xl space-y-6 pt-4 pb-16">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-2xl font-bold text-white flex items-center gap-2">
             <Map className="w-6 h-6 text-amber-500" /> Quiz Modules
          </h2>
          <button 
            onClick={handleExit} 
            className="flex items-center gap-1 rounded-xl bg-[#0f172a] border border-white/10 px-3 py-1.5 font-sans text-xs font-bold text-white/60 hover:bg-[#0f172a]/5 transition shadow-sm"
          >
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center p-8 rounded-3xl border-2 transition-all duration-300 hover:-translate-y-1 ${
                  isLocked 
                    ...
                    : isInProgress
                    ? "border-amber-500 bg-amber-500/100/10 shadow-lg"
                    : "border-emerald-500/30 bg-emerald-500/100/10/50 hover:shadow-md"
                }`}
              >
                <div className={`w-12 h-12 mb-3 flex items-center justify-center rounded-2xl ${isLocked ? "bg-[#0f172a]/10" : isInProgress ? "bg-amber-500/100" : "bg-emerald-500/100/100"} text-white shadow-sm`}>
                  <Map className="w-6 h-6" />
                </div>
                <span className={`font-display text-xl font-bold text-center ${isLocked ? "text-slate-400" : "text-white"}`}>{title}</span>
                <span className={`font-sans text-xs font-semibold mt-1 ${isLocked ? 'text-slate-500' : 'text-white/70'}`}>
                  Levels {startLevelNum} - {endLevelNum}
                </span>
                <div className="mt-4">
                  {isLocked ? <Lock className="w-5 h-5 text-slate-500" /> : 
                   isCompleted ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : 
                   <span className="text-xs font-bold text-amber-500 bg-amber-500/100/20 px-3 py-1 rounded-full">In Progress</span>}
                </div>
              </button>
            );
          })}
      ...
      <div className="mx-auto max-w-4xl space-y-6 pt-4 pb-16">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display text-2xl font-bold text-white flex items-center gap-2">
               <Map className="w-6 h-6 text-amber-500" /> {mod.title || `Module ${activeModule}`} Levels
            </h2>
          </div>
          <button 
            onClick={handleExit} 
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center p-6 rounded-2xl border transition-all duration-300 ${
                  isCurrent
                    ? "border-amber-500 bg-amber-500/100/10 ring-2 ring-amber-500/50 shadow-lg hover:-translate-y-1"
                    : isCompleted
                    ? "border-emerald-500/30 bg-emerald-500/100/10/50 hover:-translate-y-1"
              ...
              >
                ...
                <div className="absolute top-3 right-3">
                  ...
                  {isLocked && <svg className="h-4 w-4 text-white/40" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>}
                </div>

                <div className={`flex h-12 w-12 items-center justify-center rounded-2xl font-mono text-xl font-bold mb-2 shadow-sm ${
                  isCurrent ? 'bg-gradient-to-br from-amber-500 to-amber-500 text-white' :
                  isCompleted ? 'bg-[#0f172a] text-emerald-600 border border-emerald-100' :
                ...
                {isCurrent && (
                  <span className="mt-1 font-mono text-[9px] font-bold text-amber-500 uppercase tracking-wider">
                    Next up
                  </span>
                )}
              </button>
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 pb-16 pt-4">
      {!quizFinished ? (
        <div className="rounded-3xl border border-white/10 bg-[#0f172a] p-6 shadow-xl sm:p-10 space-y-8 relative">
          <button 
            onClick={() => setActiveLevel(null)} 
            className="absolute top-4 right-4 flex items-center gap-1 rounded-lg bg-[#0f172a]/5 px-3 py-1.5 font-sans text-xs font-bold text-white/60 hover:bg-[#0f172a]/10 transition"
          >
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-center justify-between font-mono text-xs font-bold text-white">
              <span className="flex items-center gap-1 text-amber-500">
                <Languages className="h-4 w-4 text-amber-500" /> Level {activeLevel}
              </span>
              <span>
                Question {currentIndex + 1} of {questions.length}
              </span>
            </div>
            <div className="h-2 w-full overflow-hidden rounded-full bg-[#0f172a]/10">
              <div
          ...
          <div className="space-y-2">
            <span className="inline-block rounded-full bg-[#0f172a]/5 px-3 py-1 font-mono text-xs font-semibold text-white/70 uppercase tracking-wider">
            ...
            {currentQ.type === 'fitb' ? (
              <div>
                <h2 className="font-display text-2xl font-bold text-white leading-relaxed">
                  <span className="text-amber-500">{currentQ.en}</span> <br/>
                  <span className="text-lg opacity-70 font-sans">({currentQ.ta})</span>
                </h2>
                <div className="mt-4 p-4 bg-[#F8F6F0] rounded-xl border border-white/10">
              ...
              <h2 className="font-display text-2xl font-bold text-white leading-relaxed">
                What does the Malayalam word <span className="text-amber-500">"{currentQ.en}"</span> ({currentQ.ta}) mean?
              </h2>
            )}
          </div>
          ...
          <div className="space-y-3">
            {currentQ.options.map((opt, idx) => {
              ...
              let btnStyle = "border-white/10 bg-[#0f172a] text-white hover:bg-[#F8F6F0]";
              if (isAnswered) {
                if (isCorrect) btnStyle = "border-emerald-500 bg-emerald-500/100/10 text-emerald-900 font-semibold";
                else if (isSelected) btnStyle = "border-red-500 bg-red-50 text-red-900";
                else btnStyle = "border-white/10 opacity-50";
              }
          ...
          {isAnswered && (
            <div className="space-y-4 rounded-2xl bg-[#F8F6F0] p-5 border border-white/10 animate-fade-in">
              ...
              <button
                onClick={handleNext}
                className="flex items-center justify-center gap-2 w-full rounded-xl bg-[#0f172a] py-3 font-sans text-sm font-bold text-white shadow-md hover:bg-[#0f172a]/90"
              >
        ...
        <div className="rounded-3xl border border-white/10 bg-[#0f172a] p-8 text-center shadow-2xl space-y-6 sm:p-12">
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-[#0f172a] text-amber-500 shadow-xl">
            <Award className="h-10 w-10" />
          </div>

          <div className="space-y-2">
            <span className="inline-block rounded-full bg-emerald-500/100/100/10 px-3 py-1 font-mono text-xs font-bold text-emerald-600">
              Level {activeLevel} Evaluation
            </span>
            <h2 className="font-display text-3xl font-bold text-white">
              You Completed Level {activeLevel}!
            </h2>
            <p className="font-sans text-sm text-white/70 max-w-md mx-auto">
          ...
          <div className="grid grid-cols-3 gap-4 rounded-2xl bg-[#F8F6F0] p-4 text-center">
            ...
            <div>
              <p className="font-mono text-2xl font-bold text-amber-500">+{passed ? 50 : 10} XP</p>
              <p className="text-[11px] text-white/60">Earned XP</p>
            </div>
            <div>
              <p className="font-mono text-2xl font-bold text-emerald-500">{scoreCount}/{questions.length}</p>
              <p className="text-[11px] text-white/60">Correct</p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            <button
              ...
              className="flex items-center justify-center gap-2 w-full sm:w-auto rounded-xl border border-white/15 bg-[#0f172a] px-6 py-3 font-sans text-sm font-bold text-white hover:bg-[#F8F6F0]"
            >
            ...
            <button
              onClick={() => setActiveLevel(null)}
              className="flex items-center justify-center gap-2 w-full sm:w-auto rounded-xl bg-[#0f172a] px-6 py-3 font-sans text-sm font-bold text-white shadow-md hover:bg-[#0f172a]/90"
            >
              <Map className="h-4 w-4 text-amber-500" /> View All Levels
            </button>
          </div>
        </div>
      )}
