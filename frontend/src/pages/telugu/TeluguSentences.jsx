...
function WordCard({ word, playAudio, index, isCompleted, isInProgress, isLocked, onInteract }) {
  ...
  return (
    <div className={`group relative flex flex-col p-5 bg-slate-900/50 backdrop-blur-xl rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-xl overflow-hidden h-full ${
      isInProgress ? "border-amber-500 ring-2 ring-amber-500/30 shadow-amber-500/10 shadow-lg" : 
      isCompleted ? "border-emerald-500/30 bg-emerald-950/20" : 
      "border-white/[0.06]"
    }`}>
      <div className="absolute inset-0 bg-gradient-to-br from-white/[0.03] to-transparent opacity-0 transition-opacity group-hover:opacity-100 pointer-events-none" />
      
      {/* Top action/status bar */}
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-1">
          {isCompleted && <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
          {isInProgress && <Play className="w-4 h-4 text-amber-400 animate-pulse" />}
          {isLocked && <Lock className="w-4 h-4 text-slate-600" />}
        </div>
        <button 
          onClick={(e) => { e.stopPropagation(); onInteract(); }}
          disabled={isLocked}
          className={`p-1.5 rounded-xl border transition-all z-10 hover:scale-110 active:scale-95 ${
            isLocked ? "bg-white/[0.03] border-white/[0.05] cursor-not-allowed opacity-40" : "bg-white/[0.05] border-white/[0.08] hover:bg-amber-500/10 hover:border-amber-500/30"
          }`}
        >
          <Volume2 className={`w-4 h-4 ${isLocked ? "text-slate-600" : "text-slate-400 group-hover:text-amber-400 transition-colors"}`} />
        </button>
      </div>

      <div className="flex-1">
        <span className="text-[22px] font-bold font-telugu leading-[1.7] tracking-wide text-white mb-3 pr-2 flex items-start gap-2 break-words">
          {word.number && (
            <span className="mt-1 flex-shrink-0 bg-amber-500/10 border border-amber-500/20 text-amber-400 px-2 py-0.5 rounded-lg text-xs font-mono font-bold">
              {word.number}.
            </span>
          )}
          <span className={isLocked ? "blur-[2px] opacity-40" : ""}>{word.telugu}</span>
        </span>
        
        <div className={`flex flex-wrap gap-2 mb-5 ${isLocked ? "opacity-30" : ""}`}>
          <span className="font-mono text-[11px] font-medium bg-white/[0.05] border border-white/[0.08] text-slate-400 px-2.5 py-1 rounded-lg transition-colors group-hover:bg-white/[0.08]">
            {word.transliteration}
          </span>
          {word.tamil_transliteration && (
            <span className="font-sans text-[11px] font-medium bg-white/[0.05] border border-white/[0.08] text-slate-400 px-2.5 py-1 rounded-lg transition-colors group-hover:bg-white/[0.08]">
              {word.tamil_transliteration}
            </span>
          )}
        </div>
      ...
      <div className="mt-auto border-t border-white/[0.06] pt-4">
        {!revealed ? (
          <button 
            ...
            className={`flex items-center justify-center gap-2 w-full py-2.5 text-xs font-bold tracking-wide rounded-xl transition-all ${
              isLocked 
                ...
                : "text-slate-400 bg-white/[0.05] hover:bg-amber-500/10 hover:text-amber-300 border border-white/[0.06] hover:border-amber-500/30"
            }`}
          >
          ...
          <div 
            ...
            className="flex flex-col gap-1.5 cursor-pointer group/reveal p-3 -mx-3 -mb-3 rounded-xl hover:bg-white/[0.04] transition-colors relative"
          >
            <div className="flex justify-between items-start pr-8">
              <span className="font-sans font-bold text-sm text-white leading-tight">
                {word.english}
              </span>
              <EyeOff className="absolute top-3.5 right-3 w-4 h-4 text-slate-500 group-hover/reveal:text-white transition-colors" />
            </div>
    ...
    <div className="flex h-screen bg-[#050816] overflow-hidden font-sans text-white">
      ...
      <aside className="w-64 h-screen bg-[#050816]/95 border-r border-white/[0.08] flex flex-col backdrop-blur-2xl shrink-0">
        ...
        <div className="p-5 border-b border-white/[0.06] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-extrabold text-white tracking-tight leading-none block">Mozhify</span>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto bg-[#050816] custom-scrollbar">
        <div className="p-6 sm:p-8 max-w-6xl mx-auto w-full space-y-6 pb-16">
            ...
            {vowels.subsets.map((subset, subsetIdx) => (
              <div key={subset.type} className="space-y-5">
                <h3 className="font-display text-2xl font-bold text-white flex items-center gap-2">
                  <Languages className="w-6 h-6 text-amber-400" /> {subset.type}
                </h3>
                  ...
                  {subset.letters.map((letter, idx) => {
                    ...
                    <div 
                      ...
                      className={`group relative flex flex-col items-center justify-between aspect-square p-3 rounded-2xl border transition-all duration-300 overflow-hidden ${
                        isInProgress
                          ? "border-amber-500 bg-amber-500/10 ring-2 ring-amber-500/40 shadow-lg shadow-amber-500/20 cursor-pointer"
                          : isCompleted
                          ? "border-emerald-500/30 bg-emerald-950/20 cursor-pointer"
                          : "border-white/[0.06] bg-slate-900/40 opacity-60 cursor-not-allowed"
                      } ${!isLocked ? 'hover:-translate-y-1 hover:shadow-lg hover:border-white/20' : ''}`}
                    >
                      <div className="flex w-full items-center justify-between z-10">
                         <span className="font-mono text-[10px] font-bold text-slate-500">{globalIdx + 1}</span>
                         {isCompleted && <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
                         {isInProgress && <Play className="w-4 h-4 text-amber-400 fill-amber-400 animate-bounce" />}
                         {isLocked && <Lock className="w-4 h-4 text-slate-600" />}
                      </div>

                      <div className="absolute inset-0 bg-gradient-to-b from-white/[0.04] to-transparent opacity-0 transition-opacity group-hover:opacity-100 pointer-events-none" />
                      {!isLocked && <Volume2 className="absolute top-7 right-2.5 w-3.5 h-3.5 text-slate-600 group-hover:text-amber-400 transition-colors" />}
                      
                      <div className="my-1 flex h-12 w-12 items-center justify-center rounded-xl bg-white/[0.06] border border-white/[0.08]">
                        <span className={`text-[36px] font-bold font-telugu leading-none ${
                          isLocked ? 'text-slate-600' : isCompleted ? 'text-emerald-300 group-hover:text-emerald-200' : 'text-white group-hover:text-amber-300'
                        } transition-colors`}>
                          {letter.char}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col items-center gap-1 z-10 w-full px-1">
                        <span className="font-mono text-[10px] font-semibold text-slate-500 w-full text-center truncate">
                        ...
                        <div className="flex justify-center gap-0.5 mt-0.5">
                          {isCompleted ? (
                             ...
                             <span className="font-mono text-[9px] font-bold text-amber-400">In Progress</span>
                           ) : (
          ...
          <div className="space-y-6">
             <h3 className="font-display text-2xl font-bold text-white flex items-center gap-2">
                <Languages className="w-6 h-6 text-emerald-400" /> Basic Consonants
             </h3>
             <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4">
                {consonants.letters.map((letter, idx) => {
                  ...
                  <div 
                    ...
                    className={`group relative flex flex-col items-center justify-between aspect-square p-3 rounded-2xl border transition-all duration-300 overflow-hidden ${
                      isInProgress
                        ? "border-emerald-500 bg-emerald-950/30 ring-2 ring-emerald-500/40 shadow-lg shadow-emerald-500/20 cursor-pointer"
                        : isCompleted
                        ? "border-emerald-500/20 bg-emerald-950/20 cursor-pointer"
                        : "border-white/[0.06] bg-slate-900/40 opacity-60 cursor-not-allowed"
                    } ${!isLocked ? 'hover:-translate-y-1 hover:shadow-lg hover:border-white/20' : ''}`}
                  >
                    <div className="flex w-full items-center justify-between z-10">
                       ...
                       {isInProgress && <Play className="w-3.5 h-3.5 text-emerald-400 fill-emerald-400 animate-bounce" />}
                       {isLocked && <Lock className="w-3.5 h-3.5 text-slate-600" />}
                    </div>

                    <div className="absolute inset-0 bg-gradient-to-b from-white/[0.04] to-transparent opacity-0 transition-opacity group-hover:opacity-100 pointer-events-none" />
                    {!isLocked && <Volume2 className="absolute top-6 right-2 w-3 h-3 text-slate-600 group-hover:text-emerald-400 transition-colors" />}
                    
                    <div className="my-0.5 flex h-10 w-10 items-center justify-center rounded-xl bg-white/[0.06] border border-white/[0.08]">
                      <span className={`text-[28px] font-bold font-telugu leading-none ${
                        isLocked ? 'text-slate-600' : isCompleted ? 'text-emerald-300 group-hover:text-emerald-200' : 'text-white group-hover:text-emerald-300'
                      } transition-colors`}>
                        {letter.char}
                      </span>
                    </div>
                    
                    <div className="flex flex-col items-center gap-0.5 z-10 w-full px-0.5">
                      <span className="font-mono text-[9px] font-semibold text-slate-500 w-full text-center truncate">
                        {letter.transliteration}
                      </span>
                      <div className="flex justify-center gap-0.5">
                        {isCompleted ? (
                           ...
                           <span className="font-mono text-[8px] font-bold text-emerald-400">In Progress</span>
                         ) : (
                           <span className="font-mono text-[8px] text-slate-600">Locked</span>
                         )}
                      </div>
                    </div>
                  </div>
        ...
        {activeTab === "Essential Words" && (
          <div className="space-y-6">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-all ${
                    activeWordCategory === category 
                      ...
                      : "bg-white/[0.05] border border-white/[0.08] text-slate-400 hover:bg-white/[0.09] hover:text-white"
                  }`}
                >
                  {category}
                </button>
            ...
            <div className="space-y-4 pt-4 border-t border-white/[0.06]">
              <h3 className="font-display text-xl font-bold text-white flex items-center gap-2">
                <BookOpen className="w-5 h-5 text-emerald-400" /> {activeWordCategory}
              </h3>
         ...
         {activeTab === "Numbers (1-100)" && (
           <div className="space-y-4">
             <h3 className="font-display text-xl font-bold text-white flex items-center gap-2">
               <BookOpen className="w-5 h-5 text-emerald-400" /> 1 to 100
             </h3>
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-8 pb-20 w-full max-w-4xl mx-auto">
              ...
              >
                ...
                <input
                  ...
                  className="w-full bg-white/[0.06] backdrop-blur-md border border-white/[0.08] rounded-2xl py-4 pl-12 pr-4 font-sans text-base focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:border-amber-500/40 transition-all text-white placeholder:text-slate-500"
                />
              </motion.div>
              ...
              <div className="flex flex-col gap-4">
                  {!searchQuery.trim() && activeSentenceModuleView === null ? (
                    <div className="space-y-4 pt-4">
                      <h3 className="font-display text-xl font-bold text-white flex items-center gap-2">
                        <BookOpen className="w-5 h-5 text-amber-400" /> {teluguSentencesData.total_sentences} Sentences
                      </h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {teluguSentencesData.modules.map((moduleData, i) => (
                          <button
                            ...
                            className="flex flex-col items-center justify-center p-8 rounded-3xl border transition-all duration-300 hover:-translate-y-1 border-white/[0.08] bg-slate-900/50 hover:border-amber-500/50 hover:bg-amber-500/5 hover:shadow-lg hover:shadow-amber-500/10"
                          >
                            ...
                            {moduleData.category && <span className="text-sm text-slate-500 mt-2 text-center">{moduleData.category}</span>}
                            <div className="mt-4">
                              <span className="text-xs font-bold text-amber-400 bg-amber-500/10 border border-amber-500/20 px-3 py-1 rounded-full">{moduleData.total_sentences || moduleData.sentences.length} Sentences</span>
                            </div>
                          </button>
                        ))}
                      </div>
                    ...
                    <>
                      {!searchQuery.trim() && activeSentenceModuleView !== null && (
                        <div className="mb-2">
                          <button 
                            ...
                            className="flex items-center gap-2 text-sm font-bold text-slate-400 hover:text-white transition-colors bg-white/[0.05] px-4 py-2 rounded-xl border border-white/[0.08] w-fit hover:bg-white/[0.09]"
                          >
                      ...
                      {filteredData.length === 0 ? (
                        <div className="text-center py-10">
                          <p className="text-slate-500 font-sans text-lg">No matches found for "{searchQuery}"</p>
                        </div>
                      ) : (
                        filteredData.map((data, index) => {
                          ...
                          return (
                            <motion.div 
                              ...
                              key={data.phase} 
                              className={`rounded-3xl border ${isActive ? 'border-white/20 shadow-xl bg-slate-800/80 backdrop-blur-md' : 'border-white/[0.07] bg-slate-900/50 backdrop-blur-sm'} transition-all duration-300 overflow-hidden`}
                            >
                              {/* Accordion Header */}
                              <button
                                onClick={() => togglePhase(data.phase)}
                                className="w-full flex items-center justify-between p-6 sm:p-8 text-left hover:bg-white/[0.04] transition-colors"
                              >
                                <div>
                                  <h2 className={`font-display text-2xl font-extrabold ${isActive ? 'text-white' : 'text-slate-300'}`}>
                                    {data.phase} {completedPhases[data.phase] && <span className="ml-2 inline-flex items-center text-sm font-bold text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded-full">⭐ Passed</span>}
                                  </h2>
                                ...
                                <div className={`p-3 rounded-full transition-colors ${isActive ? 'bg-white/10' : 'bg-transparent'}`}>
                                  {isActive ? <ChevronUp className="h-6 w-6 text-white" /> : <ChevronDown className="h-6 w-6 text-slate-500" />}
                                </div>
                              </button>
                                    ...
                                    <div className="p-6 sm:p-8 pt-6">
                                      ...
                                      <div className="space-y-4">
                                        {data.sentences.map((sentence, sIndex) => {
                                          ...
                                          return (
                                            <div key={sIndex} className="group flex flex-col sm:flex-row gap-4 sm:gap-6 p-5 rounded-2xl bg-white/[0.04] border border-white/[0.07] hover:border-amber-500/20 hover:bg-white/[0.07] transition-all duration-300">
                                              <div className="flex-shrink-0 mt-1">
                                                <div className="h-10 w-10 rounded-full bg-white/[0.06] flex items-center justify-center text-slate-500 group-hover:bg-amber-500/10 group-hover:text-amber-400 transition-colors">
                                              ...
                                              <div className="flex-1 space-y-3">
                                                <div className="flex items-start justify-between gap-4">
                                                  <p className="font-serif text-xl sm:text-2xl text-white font-medium leading-snug">
                                                    {sentence.telugu}
                                                  </p>
                                                  <button 
                                                    onClick={() => playAudio(sentence.telugu)}
                                                    className="p-2.5 rounded-full hover:bg-white/[0.08] text-slate-500 hover:text-amber-400 transition-colors flex-shrink-0"
                                                  >
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                  <button
                                                    ...
                                                    className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 border border-amber-500/20 transition-colors"
                                                  >
                                                ...
                                                <AnimatePresence>
                                                  {isTranslated && (
                                                    ...
                                                    >
                                                      ...
                                                      <p className="font-sans text-slate-500 text-sm italic font-mono bg-white/[0.04] border border-white/[0.06] p-2 rounded-lg mt-2">
