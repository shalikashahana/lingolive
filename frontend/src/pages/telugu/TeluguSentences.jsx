...
import { 
  ...
  Play, Volume2, Eye, EyeOff, User, Filter, LogOut, Lock, Star, Flame, Zap, BarChart3, Globe, LayoutDashboard 
} from "lucide-react";
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-10 shadow-sm border border-[#14213D]/5">
        ...
        {activeTab === "Essential Words" && (
         ...
         {activeTab === "Sentences" && (
            <div className="space-y-6">
              {!activeSentenceModule ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {Object.keys(groupedSentencesByModule).map((moduleName, i) => {
                    const startIdx = i * 500;
                    const isLocked = progress.sentences < startIdx;
                    const isCompleted = progress.sentences >= (i + 1) * 500;
                    ...
                    return (
                      <button
                        key={moduleName}
                        disabled={isLocked}
                        onClick={() => setActiveSentenceModule(moduleName)}
                      ...
                      >
                        ...
                        <span className={`font-display text-lg font-bold ${isLocked ? "text-gray-500" : "text-[#14213D]"}`}>{moduleName}</span>
                        <div className="mt-3">
              ...
              ) : !activeSentencePartView ? (
                <div className="space-y-6">
                  <button 
                    onClick={() => setActiveSentenceModule(null)}
                  ...
                  <div className="flex items-center justify-between">
                    <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                      <BookOpen className="w-6 h-6 text-[#3F6656]" /> {activeSentenceModule}
                    </h3>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {Object.keys(groupedSentencesByModule[activeSentenceModule]).map((category) => {
                      const setNumStr = category.match(/Part (\d+)/)[1];
                      const setNum = parseInt(setNumStr, 10);
                      const startIdx = (setNum - 1) * 10;
                      const isLocked = progress.sentences < startIdx;
                      const isCompleted = progress.sentences >= setNum * 10;
                      const isInProgress = !isLocked && !isCompleted;
                      ...
                      return (
                        <button
                          key={category}
                          disabled={isLocked}
                          onClick={() => setActiveSentencePartView(category)}
                          className={`flex flex-col items-center justify-center p-8 rounded-3xl border-2 transition-all duration-300 hover:-translate-y-1 ${
                            isLocked 
                              ? "border-[#14213D]/10 bg-gray-50/60 opacity-70 cursor-not-allowed" 
                              : isInProgress
                        ...
                        >
                          <BookOpen className={`w-8 h-8 mb-3 ${isLocked ? "text-gray-400" : isInProgress ? "text-[#C9A227]" : "text-emerald-500"}`} />
                          <span className={`font-display text-lg font-bold ${isLocked ? "text-gray-500" : "text-[#14213D]"}`}>{category}</span>
                          <div className="mt-3">
                            {isLocked ? <Lock className="w-5 h-5 text-gray-400" /> : 
                             isCompleted ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : 
                             <span className="text-xs font-bold text-[#C9A227] bg-[#C9A227]/20 px-3 py-1 rounded-full">In Progress</span>}
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <h3 className="font-display text-2xl font-bold text-[#14213D] flex items-center gap-2">
                      <BookOpen className="w-6 h-6 text-[#3F6656]" /> {activeSentencePartView}
                    </h3>
                  </div>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {groupedSentencesByModule[activeSentenceModule][activeSentencePartView].map((sentence) => {
                       const idx = sentence.id - 1;
                       const isCompleted = idx < progress.sentences;
                       const isInProgress = idx === progress.sentences;
                       const isLocked = idx > progress.sentences;
                       return (
                         <WordCard 
                           ...
                           isInProgress={isInProgress}
                           isLocked={isLocked}
                           onInteract={() => handleInteraction('sentences', idx, sentence.telugu)} 
                         />
                       );
                    })}
                  </div>
