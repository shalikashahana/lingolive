...
const TELUGU_SCENARIOS = [
  {
    ...
    icon: MessageSquare,
    promptPrefix: "",
    welcome: () => `నమస్కారం! నేను Gemma, మీ AI Telugu Conversation Coach! ఇప్పుడు మనం Telugu లో మాట్లాడుదాం. మీరు ఏదైనా చెప్పండి — నేను మీకు feedback మరియు corrections ఇస్తాను! (Hello! Let us practice Telugu together!)`,
  },
  {
    id: "market",
    ...
    welcome: () => `బజారుకు స్వాగతం! (Welcome to the market!) మీకు ఏమి కావాలి? (What would you like to buy?) Let's practice shopping conversations in Telugu!`,
  },
  {
    id: "family",
    ...
    welcome: () => `కుటుంబం గురించి మాట్లాడుదాం! (Let's talk about family!) మీ కుటుంబంలో ఎంత మంది ఉన్నారు? (How many people are in your family?)`,
  },
  {
    id: "interview",
    label: "Job Interview",
    ...
    welcome: () => `ఉద్యోగ ఇంటర్వ్యూకు స్వాగతం! (Welcome to Job Interview Practice!) మీరు మీ గురించి Telugu లో చెప్పండి. (Please introduce yourself in Telugu.)`,
  },
];
...
const TELUGU_SYSTEM_PROMPT = `You are Gemma, a friendly AI Telugu Conversation Coach. Your goal is to have natural, engaging Telugu conversations with the user.

In every reply:
1. Respond naturally in BOTH Telugu (తెలుగు) and English (for context/learning).
2. If the user makes any Telugu grammar or vocabulary mistake, gently correct it.
3. Suggest a more natural Telugu phrasing if needed.
4. End with a follow-up question in Telugu (with English translation).
Be warm, encouraging, and never condescending.

Output your response ONLY as JSON in this exact format:
{
  "reply": "Your Telugu response here (తెలుగు లో జవాబు) with English context in parentheses, ending with a follow-up question.",
  "corrections": {
    "original": "the exact phrase the user used that needs improvement",
    "improved": "the better Telugu phrasing",
...
export default function TeluguChat() {
  ...
  const quickChips = [
    "నమస్కారం! (Hello!)",
    "నాకు Telugu నేర్పించు (Teach me Telugu)",
    "నా grammar సరి చేయి (Correct my grammar)",
    "Simple conversation చేద్దాం (Let's have a simple chat)"
  ];
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-amber-500/20 bg-gradient-to-r from-amber-900/30 via-[#0f172a] to-[#050816] px-6 py-4 backdrop-blur-xl z-10">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-slate-400">తెలుగు Conversation Coach • Live Voice & Text</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSend} className="border-t border-white/10 bg-[#0f172a]/95 p-4 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          <button
            ...
            title={isListening ? "Listening…" : "Speak in Telugu"}
          >
          ...
          <input
            ...
            placeholder={isListening ? "వింటున్నాను… (Listening…)" : "Telugu లో మాట్లాడండి లేదా type చేయండి…"}
