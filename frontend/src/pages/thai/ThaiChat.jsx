...
const SCENARIOS = [
  { 
    ...
    icon: MessageSquare, 
    promptPrefix: "[Thai Language Tutor Mode] You are teaching Thai to an English speaker. For every reply, provide the Thai (Thai script) first, followed by the English transliteration in parentheses, and then the English translation. Example format: 'สวัสดีค่ะ! (Sawasdee ka!) - Hello!'. Gently correct any mistakes. User says: ",
    welcome: () => `สวัสดีค่ะ! (Sawasdee ka!) - Hello! I am Gemma, your Thai conversation coach. You can speak in English, and I will help you translate it to Thai, or you can try speaking in Thai!`
  },
  { 
    id: "interview", 
    label: "Job Interview Roleplay", 
    icon: Briefcase, 
    promptPrefix: "[Thai Job Interview Tutor Mode] You are teaching formal Thai to an English speaker. Provide the Thai, then (transliteration), then - English translation. User says: ",
    welcome: () => `ยินดีต้อนรับสู่การสัมภาษณ์! (Yindee ton rap su kan samphat!) - Welcome to the interview practice! I will act as your hiring manager. Tell me about yourself.`
  },
  { 
    id: "business", 
    label: "Business Presentation", 
    icon: Award, 
    promptPrefix: "[Thai Business Tutor Mode] You are teaching professional Thai to an English speaker. Provide the Thai, then (transliteration), then - English translation. User says: ",
    welcome: () => `ยินดีต้อนรับสู่การนำเสนอธุรกิจ! (Yindee ton rap su kan nam saner thurakit!) - Welcome to the Business Presentation practice! What would you like to present today?`
  },
  { 
    id: "travel", 
    label: "Travel & Dining Out", 
    icon: Compass, 
    promptPrefix: "[Thai Travel Tutor Mode] You are teaching conversational travel Thai to an English speaker. Provide the Thai, then (transliteration), then - English translation. User says: ",
    welcome: () => `ยินดีต้อนรับสู่การท่องเที่ยว! (Yindee ton rap su kan thong thiao!) - Welcome to Travel & Dining Out practice! Imagine you're at a hotel in Bangkok. What do you say?`
  },
];

export default function ThaiChat() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("lingolive_thai_chat_history");
      if (saved) {
  ...
  const saveSessionToHistory = (currentMsgs, scenarioId, currentId) => {
    ...
    setHistoryList((prev) => {
      ...
      try {
        localStorage.setItem("lingolive_thai_chat_history", JSON.stringify(updated));
      } catch (e) {
  ...
  const handleClearAllHistory = () => {
    if (window.confirm("Clear all chat history?")) {
      setHistoryList([]);
      localStorage.removeItem("lingolive_thai_chat_history");
    }
  };
  ...
  const handleSendMessage = async (e, customText = null) => {
    ...
    try {
      ...
      // Attaching the provided API Key per user request for Thai realtime
      const API_KEY = import.meta.env.VITE_THAI_REALTIME_API_KEY || "";
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-slate-400">
              Real-Time Thai Conversation Coach • Live Voice &amp; Text
            </p>
          </div>
        </div>
      ...
      <form onSubmit={(e) => handleSendMessage(e)} className="border-t border-white/10 bg-[#0f172a]/95 p-4 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            placeholder={isListening ? "Listening... speak now..." : "Type your sentence in English or Thai…"}
