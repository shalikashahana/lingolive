...
export default function ThaiChat() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("mozhify_thai_chat_history");
      if (saved) {
  ...
  const saveSessionToHistory = (currentMsgs, scenarioId, currentId) => {
    ...
    setHistoryList((prev) => {
      ...
      try {
        localStorage.setItem("mozhify_thai_chat_history", JSON.stringify(updated));
      } catch (e) {
  ...
  const handleClearAllHistory = () => {
    if (window.confirm("Clear all chat history?")) {
      setHistoryList([]);
      localStorage.removeItem("mozhify_thai_chat_history");
    }
  };
