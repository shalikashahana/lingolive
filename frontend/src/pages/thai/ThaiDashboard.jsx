...
import ThaiChat from "./ThaiChat";
import ThaiQuiz from "./ThaiQuiz";

function ProgressRing({ progress, size = 44, strokeWidth = 4, color = "#F59E0B" }) {
  ...
  return (
    <motion.div 
      ...
      className={`group relative flex flex-col p-5 bg-[#0f172a]/90 backdrop-blur-xl rounded-3xl border transition-all duration-300 overflow-hidden h-full shadow-lg ${
        isInProgress ? "border-amber-500 ring-2 ring-amber-500/30 bg-blue-950/20" : 
    ...
    >
      ...
      <div className="flex-1">
        <div className="text-2xl font-bold font-sans text-white mb-2 flex items-start gap-2 break-words">
          ...
          <span className={isLocked ? "blur-[3px] opacity-50" : "text-blue-100"}>{word.thai || word.thai_numeral || word.thai_word}</span>
        </div>
        
        <div className={`flex flex-wrap gap-2 mb-4 ${isLocked ? "opacity-40" : ""}`}>
          {(word.english_transliteration || word.thai_word) && (
            <span className="font-mono text-[11px] font-semibold bg-white/5 border border-white/10 text-amber-300 px-2.5 py-1 rounded-xl">
              {word.english_transliteration || word.thai_word}
            </span>
          )}
          {(word.tamil_transliteration || word.tamil_pronunciation) && (
...
export default function ThaiDashboard() {
  ...
  const formattedThaiSentences = useMemo(() => {
    const parts = [];
    if (Array.isArray(thaiSentencesData)) {
      const sentences = thaiSentencesData;
    ...
    } else if (thaiSentencesData.modules) {
      thaiSentencesData.modules.forEach((mod, modIndex) => {
  ...
  const [completedPhases, setCompletedPhases] = useState(() => {
    const saved = localStorage.getItem("thai_completed_phases");
  ...
  const handleCheckpointComplete = (score, total) => {
    if (checkpointPhase) {
      ...
      localStorage.setItem("thai_completed_phases", JSON.stringify(updated));
    }
  };

  const filteredData = useMemo(() => {
    let dataToFilter = formattedThaiSentences;
    if (!searchQuery.trim()) {
    ...
    return dataToFilter.map(phaseObj => {
      const matchingSentences = phaseObj.sentences.filter(s => {
        ...
        const thMatch = s.thai.includes(searchQuery);
        return enMatch || taMatch || thMatch;
      });
      return { ...phaseObj, sentences: matchingSentences };
    }).filter(phaseObj => phaseObj.sentences.length > 0);
  }, [searchQuery, formattedThaiSentences, activeSentenceModuleView, activeSentenceLevelView]);
  ...
  // Thai Royal Blue + Gold Visual Theme Cards
  const dashboardCards = [
    { key: "swarangal", label: "Consonants (พยัญชนะ)", native: "พยัญชนะ", tab: TABS[1], icon: "ก", total: alphabetData.consonants.length, color: "#1E3A8A", estimatedTime: "25 mins", difficulty: "Beginner" },
    { key: "words", label: "Essential Words", native: "คำศัพท์", tab: TABS[2], icon: "📚", total: thaiWordsData.length, color: "#0EA5E9", estimatedTime: "45 mins", difficulty: "Beginner" },
    { key: "numbers", label: "Numbers", native: "ตัวเลข", tab: TABS[3], icon: "🔢", total: thaiNumbersData.length, color: "#EC4899", estimatedTime: "25 mins", difficulty: "Beginner" },
    { key: "sentences", label: "Sentences", native: "ประโยค", tab: TABS[4], icon: "💬", total: thaiSentencesData.total_sentences || thaiSentencesData.length || 0, color: "#10B981", estimatedTime: "60 mins", difficulty: "Intermediate" },
    { key: "quiz", label: "Quiz Dashboard", native: "แบบทดสอบ", tab: TABS[5], icon: "🧠", total: thaiQuizData.total_questions, color: "#8B5CF6", estimatedTime: "20 mins", difficulty: "All Levels" }
  ];
      ...
      {lettersArray.map((letter, idx) => {
        ...
        return (
          <motion.div 
            ...
            onClick={() => !isLocked && handleInteraction(type, idx, letter.thai_script || letter.letter)}
          ...
          >
            ...
            <div className="my-1 flex h-12 w-12 items-center justify-center rounded-2xl font-mono text-base font-bold bg-white/5 text-blue-300 border border-white/10 shadow-sm group-hover:border-amber-500/40 transition-colors">
              <span className={`text-3xl font-bold font-sans ${isLocked ? 'text-slate-600' : 'text-amber-200 group-hover:text-amber-400'} transition-colors`}>
                {letter.thai_script || letter.letter}
              </span>
            </div>
            ...
            <div className="flex flex-col items-center gap-1 z-10 w-full px-1">
              <span className="font-mono text-[10px] font-semibold bg-white/5 text-amber-300 px-1.5 py-0.5 rounded-lg w-full text-center truncate border border-white/5">
                {letter.thai_name || letter.transliteration}
              </span>
            </div>
          </motion.div>
        );
      ...
      <aside className="w-64 h-screen bg-[#090d1f]/95 border-r border-blue-500/10 flex flex-col backdrop-blur-2xl shrink-0">
        <div className="p-5 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-tr from-blue-700 to-amber-400 text-white shadow-lg shadow-blue-600/20 font-bold">
                ...
                {availableLanguages.map((lang) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 px-4 py-2.5 font-sans text-sm font-semibold transition-colors ${
                      currentLanguageCode === lang.code ? "bg-blue-500/10 text-amber-400" : "text-slate-300 hover:bg-white/5"
                    }`}
                  >
                    <span>{lang.flag}</span>
                    <span>{lang.name}</span>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-1 custom-scrollbar">
          {TABS.map((tabName) => {
            const getIcon = (name) => {
              ...
              if (name.includes("Alphabets")) return <span className="text-sm font-sans font-bold">ก</span>;
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-2xl font-semibold text-sm transition-all duration-200 ${
                  isActive
                    ? "bg-gradient-to-r from-blue-800/30 to-amber-500/10 border border-blue-500/40 text-amber-300 shadow-sm"
              ...
              >
                ...
                {isActive && <div className="h-2 w-2 rounded-full bg-amber-400 shadow-[0_0_8px_#f59e0b] shrink-0" />}
              </button>
            );
          })}
        </nav>
          ...
          {user && (
            <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-2.5">
              <div className="flex items-center gap-2.5 overflow-hidden">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-tr from-blue-700 to-amber-400 text-xs font-bold text-white">
                  {user.displayName?.[0] || user.email?.[0]?.toUpperCase() || "T"}
                </div>
                <div className="overflow-hidden">
                  <p className="text-xs font-bold text-white truncate">{user.displayName || "Thai Learner"}</p>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto bg-[#050816] custom-scrollbar">
        <div className="p-6 sm:p-8 max-w-6xl mx-auto w-full space-y-8 pb-16">
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 p-6 rounded-3xl border border-blue-500/20 bg-gradient-to-br from-[#0f172a] to-[#090d1f] flex flex-col justify-between space-y-4 shadow-xl">
                    ...
                    <div className="space-y-3">
                      {[
                        { text: "Learn 5 Thai Consonants (พยัญชนะ)", tab: "Alphabets (อักษร)", done: progress.swarangal >= 5 },
                        { text: "Study 10 Thai Vocabulary Words", tab: "Essential Words", done: progress.words >= 10 },
                        { text: "Take Thai Skill Practice Quiz", tab: "Quiz Dashboard", done: false },
                      ].map((item, i) => (
                        <div
                          key={i}
                  ...
                  <div className="p-6 rounded-3xl border border-amber-500/20 bg-gradient-to-b from-amber-950/30 via-[#0f172a] to-[#050816] flex flex-col justify-between space-y-4 shadow-xl">
                    <div>
                      <div className="h-10 w-10 rounded-2xl bg-gradient-to-tr from-blue-700 to-amber-400 flex items-center justify-center text-white shadow-md mb-3 font-bold">
                        <Sparkles className="w-5 h-5" />
                      </div>
                      <h3 className="text-lg font-bold text-white mb-1">Continue Learning</h3>
                      <p className="text-xs text-slate-400 leading-relaxed">
                        Keep building Thai character recognition and tones.
                      </p>
                    </div>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-5">
                    <h2 className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">Thai Modules</h2>
                        ...
                        >
                          ...
                          <div className="w-full space-y-2">
                            ...
                            <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
                              <div
                                className="h-full rounded-full bg-gradient-to-r from-blue-700 to-amber-400 transition-all duration-700"
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          </div>
            ...
            {activeTab === "Alphabets (อักษร)" && (
              <div className="space-y-10">
                <div className="space-y-4">
                  <h3 className="text-2xl font-bold text-white flex items-center gap-2">
                    <Languages className="w-6 h-6 text-amber-400" /> Thai Consonants (พยัญชนะ)
                  </h3>
                  {renderLetterGrid("swarangal", alphabetData.consonants)}
                </div>
              </div>
            )}
                ...
                {activeWordPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-sky-400" /> {thaiWordsData.length} Essential Thai Words
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(thaiWordsData.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = (i + 1) * 10;
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {thaiWordsData.slice(activeWordPartView * 10, (activeWordPartView + 1) * 10).map((word, relIdx) => {
            ...
            {activeTab === "Numbers" && (
              <div className="space-y-6">
                {activeNumberPartView === null ? (
                  <div className="space-y-4 pt-4">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-pink-400" /> {thaiNumbersData.length} Thai Numbers
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from({ length: Math.ceil(thaiNumbersData.length / 10) }).map((_, i) => {
                        const startIdx = i * 10;
                        const endIdx = (i + 1) * 10;
                        const partName = `Part ${i + 1} (${startIdx + 1}-${endIdx})`;
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {thaiNumbersData.slice(activeNumberPartView * 10, (activeNumberPartView + 1) * 10).map((number, relIdx) => {
            ...
            {activeTab === "Sentences" && (
              <div className="space-y-8 pb-16 w-full max-w-4xl mx-auto">
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-950/40 via-[#0f172a] to-[#050816] p-8 border border-blue-500/30 shadow-2xl">
                  <h1 className="text-3xl font-extrabold text-white mb-2">Daily Conversations (ประโยค)</h1>
                  <p className="text-sm text-slate-400">Master everyday Thai sentences grouped by practical scenarios.</p>
                </div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Search sentences in Thai, English or Tamil..."
                ...
                <div className="space-y-4">
                  {activeSentenceModuleView === null && !searchQuery.trim() ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Array.from(new Set(formattedThaiSentences.map(p => p.moduleIndex))).map((moduleIdx) => {
                        const moduleData = formattedThaiSentences.filter(p => p.moduleIndex === moduleIdx);
                                ...
                                return (
                                  <div key={sIndex} className="p-5 rounded-2xl bg-[#0f172a]/80 backdrop-blur-sm border border-white/10 hover:border-amber-500/30 transition-all shadow-lg flex flex-col justify-between min-h-[140px]">
                                    <div className="flex items-start justify-between mb-4">
                                      <p className="text-xl font-bold text-amber-200 leading-relaxed pr-4">{sentence.thai}</p>
                                      <button onClick={() => playAudio(sentence.thai)} className="p-2.5 rounded-xl bg-blue-500/10 text-amber-400 hover:bg-blue-500 hover:text-white transition-all shrink-0">
            ...
            {activeTab === "Quiz" && (
              <div className="-mx-4 sm:-mx-10 -my-6 sm:-my-10">
                <ThaiQuiz onExit={() => setActiveTab(TABS[0])} />
              </div>
            )}
